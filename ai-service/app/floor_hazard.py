...
class FloorHazardAnalyzer:
    ...
    def analyze(
        ...
        *,
        ...
        people: list[PersonDetection] = []
        if detect_people:
            people_result = self.people_model.predict(image, classes=[0], conf=0.25, imgsz=960, device=DEVICE, verbose=False)[0]
            people = [] if people_result.boxes is None else [
                PersonDetection(confidence=float(score), bbox=BoundingBox(x1=box[0], y1=box[1], x2=box[2], y2=box[3]))
                for box, score in zip(people_result.boxes.xyxy.tolist(), people_result.boxes.conf.tolist())
            ]
        if not detect_floor:
            return FloorAnalysis(people=people, hazards=[], processing_time_ms=(perf_counter() - started) * 1000)
        ...
        return FloorAnalysis(people=people, hazards=hazards, processing_time_ms=(perf_counter() - started) * 1000)
