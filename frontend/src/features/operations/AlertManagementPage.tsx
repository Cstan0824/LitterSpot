...
function ProtectedEvidence({ alert, alt }: { alert: Alert; alt: string }) {
  ...
  return url ? <img src={url} alt={alt} /> : <span>No retained evidence</span>;
}
...
export function AlertManagementPage({ readOnly = false, availableCleaners = [], onAssign, onDismiss, workOrders = [], alerts, cameras, cleaners, onStatus }: Props) {
  ...
  if (selected) {
    const linkedWork = selected.activeWorkOrderId ? workOrders.find((work) => work.id === selected.activeWorkOrderId) : undefined;
