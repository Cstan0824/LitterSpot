...
import { assignV2Alert, createV2Cleaner, createV2ManualWork, dismissV2Alert, dismissV2Work, getV2CleanersPage, overrideV2Verification, reassignV2Work, takeOverV2Work, updateV2Cleaner, updateV2CleanerStation, verifyV2Work, type CreateV2CleanerInput, type V2Alert, type V2Camera, type V2Cleaner, type V2OperationsReadModel, type V2Point, type V2WeeklySchedule, type V2WorkOrder } from "../../services/v2/operations";
import { createV2RegularSupervisor, getV2SupervisorsPage, updateV2SupervisorAccount, type CreateV2RegularSupervisorInput, type V2SupervisorListItem, type V2SupervisorManagementItem } from "../../services/v2/supervisors";
...
import { invalidatePagedResources } from "../../services/v2/pagination";
...
export function adaptV2Operations(model: V2OperationsReadModel) {
  ...
  const cameras: CameraRecord[] = model.cameras.filter((camera): camera is V2Camera & { status: "active" | "inactive" } => camera.status !== "removed").map((camera) => ({ id: camera.id, siteId: model.siteMap.siteId, siteName: model.siteMap.siteName, zoneId: camera.placement?.zoneId ?? "", zoneName: zoneName.get(camera.placement?.zoneId ?? "") ?? "Unplaced", code: camera.id.startsWith("local-camera-") ? `CAM-${camera.id.slice("local-camera-".length)}`.toUpperCase() : camera.id.slice(-8).toUpperCase(), name: camera.name, sourceMode: camera.sourceType === "looped_video" ? "upload" : "stream", status: camera.status, availability: camera.runtime?.connectionStatus === "online" ? "available" : "unavailable", registrationStatus: camera.registration?.status === "ready" ? "ready" : "unregistered", registrationRevision: 0, registrationUpdatedAt: null, createdAt: null, updatedAt: null }));
...
export function OperationsConsole({ supervisor, capabilities, page, onNavigate, onLogout }: { supervisor: AdminProfile & { uid: string }; capabilities: SupervisorCapabilities; page: Page; onNavigate: (page: Page, params?: Record<string, string>) => void; onLogout: () => void }) {
  ...
  useEffect(() => { if (page !== "admin") return; const controller = new AbortController(); void Promise.all([getV2CleanersPage({}, controller.signal), getV2SupervisorsPage(undefined, controller.signal)]).then(([cleanerPage, supervisorPage]) => { setCleanerCursor(cleanerPage.nextCursor); setCleanerTotal(cleanerPage.totalCount); setSupervisorCursor(supervisorPage.nextCursor); setSupervisorTotal(supervisorPage.totalCount); }).catch(() => undefined); return () => controller.abort(); }, [page, v2Model?.cleaners.length, v2Supervisors.length]);
  const loadMoreCleaners = useCallback(async () => { if (!cleanerCursor || !modelRef.current) return; const page = await getV2CleanersPage({ cursor: cleanerCursor }); const current = modelRef.current; acceptModel({ ...current, cleaners: [...current.cleaners, ...page.items.filter((item) => !current.cleaners.some((loaded) => loaded.id === item.id))] }); setCleanerCursor(page.nextCursor); setCleanerTotal(page.totalCount); }, [acceptModel, cleanerCursor]);
  const loadMoreSupervisors = useCallback(async () => { if (!supervisorCursor) return; const page = await getV2SupervisorsPage(supervisorCursor); setV2Supervisors((current) => [...current, ...page.items.filter((item) => !current.some((loaded) => loaded.uid === item.uid))]); setSupervisorCursor(page.nextCursor); setSupervisorTotal(page.totalCount); }, [supervisorCursor]);
  useEffect(() => {
  ...
  const content = useMemo(() => {
    ...
    if (page === "dashboard" && v2Model) return <GeographicOperationsDashboard zones={zones} cameras={cameraRecords} alerts={allAlerts} workOrders={v2WorkOrders} cleaners={staff} recommendations={placementRecommendations} availabilityById={availabilityById} busyZones={v2Dashboard?.busyZones} siteMap={v2Model.siteMap} siteName={siteName} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} />;
    if (page === "admin" && v2Model) return <TeamManagementPage viewer={{ uid: supervisor.uid, authority: supervisor.authority }} staff={staff} zones={zones} v2Cleaners={v2Model.cleaners} cleanerTotal={cleanerTotal} cleanerHasMore={Boolean(cleanerCursor)} onLoadMoreCleaners={loadMoreCleaners} workOrders={v2WorkOrders} siteMap={v2Model.siteMap} supervisors={v2Supervisors} supervisorTotal={supervisorTotal} supervisorHasMore={Boolean(supervisorCursor)} onLoadMoreSupervisors={loadMoreSupervisors} supervisorsLoading={supervisorsLoading} supervisorsError={supervisorsError} onRetrySupervisors={() => { invalidateOperationsResources(["supervisors"]); void loadSupervisors(true); }} onCreateSupervisor={createSupervisorV2} onUpdateSupervisor={updateSupervisorV2} onCreate={createCleanerV2} onUpdate={updateCleanerV2} onMoveStation={moveCleanerStation} onAvailabilityOverride={overrideCleanerAvailability} />;
    if (page === "cameras") return <CameraOperationsPage {...supervisorCameraPageAccess(capabilities)} sites={sites} zones={zones} cameras={cameraRecords} v2Cameras={v2Model?.cameras} siteMap={v2Model?.siteMap} workOrders={v2WorkOrders} feeds={cameras} liveVideos={liveVideos} alerts={allAlerts} cleaners={staff} availableCleanerIds={staff.filter((cleaner) => availabilityById[cleaner.id]?.available).map((cleaner) => cleaner.id)} error={locationError} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} onWorkAction={actOnWork} onCameraPublished={async () => { await refreshAfterMutation("camera_publish"); }} onCameraRemoved={async () => { await refreshAfterMutation("camera_remove"); }} onDismissCameraWork={(workId, reason) => actOnWork(workId, "dismiss", { reason })} onCreateZone={async () => { throw new Error("Zone creation is handled by the V2 Camera registration wizard."); }} onCreateCamera={async () => { throw new Error("Camera creation is handled by the V2 Camera registration wizard."); }} />;
    if (page === "alerts") return <AlertManagementPage alerts={allAlerts} cameras={cameraRecords} cleaners={staff} workOrders={v2WorkOrders} availableCleaners={staff.filter((cleaner) => availabilityById[cleaner.id]?.available)} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} onAssign={assignAlert} onDismiss={dismissAlert} onStatus={(alert, next) => changeStatus(alert, next)} />;
    if (page === "history" && v2Model) return <WorkManagementPage siteMap={v2Model.siteMap} workOrders={v2WorkOrders} cleaners={staff} zones={zones} cameras={cameraRecords} alerts={allAlerts} availableCleanerIds={staff.filter((cleaner) => availabilityById[cleaner.id]?.available).map((cleaner) => cleaner.id)} onCreateV2Work={createManualWork} onV2Action={actOnWork} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} />;
