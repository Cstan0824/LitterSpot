...
import { assignAlertToCleaner, createCleanerAccount, createManualWorkOrder, dismissAlertRecord, dismissWorkOrder, getCleanersPage, overrideWorkVerification, reassignWorkOrder, takeOverWorkOrder, updateCleanerAccount, updateCleanerStation, verifyWorkOrder, type CreateCleanerInput, type OperationsAlert, type OperationsCamera, type OperationsCleaner, type OperationsReadModel, type OperationsPoint, type WeeklySchedule, type OperationsWorkOrder } from "../../services/api/operations";
import { createRegularSupervisor, getSupervisorsPage, updateSupervisorAccount, type CreateRegularSupervisorInput, type SupervisorListItem, type SupervisorManagementItem } from "../../services/api/supervisors";
...
import type { SupervisorCapabilities } from "../../services/api/session";
...
import { coreOperationsResources, invalidateOperationsResources, readOperationsResources, type OperationsResourceKey } from "../../services/api/operationsResourceCache";
import { resourcesAfterMutation, resourcesForPage, type OperationsMutation } from "./operationsRefreshPolicy";
import { invalidatePagedResources } from "../../services/api/pagination";
...
export function adaptOperations(model: OperationsReadModel) {
...
export function OperationsConsole({ supervisor, capabilities, page, onNavigate, onLogout }: { supervisor: AdminProfile & { uid: string }; capabilities: SupervisorCapabilities; page: Page; onNavigate: (page: Page, params?: Record<string, string>) => void; onLogout: () => void }) {
  ...
  const [operationsDashboard, setOperationsDashboard] = useState<OperationsReadModel["dashboard"]>();
  const [operationsWorkOrders, setOperationsWorkOrders] = useState<OperationsReadModel["workOrders"]>([]);
  const [operationsAlerts, setOperationsAlerts] = useState<OperationsAlert[]>([]);
  const [operationsModel, setOperationsModel] = useState<OperationsReadModel>();
  const [supervisorAccounts, setSupervisorAccounts] = useState<SupervisorListItem[]>([]);
  const [supervisorsLoading, setSupervisorsLoading] = useState(false);
  const [supervisorsError, setSupervisorsError] = useState<string>();
  const [cleanerCursor, setCleanerCursor] = useState<string | null>(null);
  const [cleanerTotal, setCleanerTotal] = useState(0);
  const [supervisorCursor, setSupervisorCursor] = useState<string | null>(null);
  const [supervisorTotal, setSupervisorTotal] = useState(0);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string>();
  const modelRef = useRef<OperationsReadModel | undefined>(undefined);
  useEffect(() => {
  ...
  const acceptModel = useCallback((model: OperationsReadModel) => {
    modelRef.current = model;
    const data = adaptOperations(model);
    setStaff(data.staff); setSites(data.sites); setZones(data.zones); setCameraRecords(data.cameras); setAllAlerts(data.alerts);
    setAvailabilityById(data.availabilityById); setStationById(data.stationById); setScheduleSummaryById(data.scheduleSummaryById); setOperationsDashboard(data.dashboard); setOperationsWorkOrders(data.workOrders); setOperationsAlerts(model.alerts);
    setOperationsModel(model); setStaffError(undefined); setLocationError(undefined);
  }, []);
  ...
  const refreshCoreResources = useCallback(async (keys: readonly OperationsResourceKey[], force = false) => {
    const coreKeys = keys.filter((key): key is keyof OperationsReadModel => key !== "supervisors");
  ...
  const loadSupervisors = useCallback(async (force = false) => {
    ...
    try { const result = await readOperationsResources(["supervisors"], { force }); setSupervisorAccounts(result.supervisors); }
  ...
  useEffect(() => { if (page !== "admin") return; const controller = new AbortController(); void Promise.all([getCleanersPage({}, controller.signal), getSupervisorsPage(undefined, controller.signal)]).then(([cleanerPage, supervisorPage]) => { setCleanerCursor(cleanerPage.nextCursor); setCleanerTotal(cleanerPage.totalCount); setSupervisorCursor(supervisorPage.nextCursor); setSupervisorTotal(supervisorPage.totalCount); }).catch(() => undefined); return () => controller.abort(); }, [page, operationsModel?.cleaners.length, supervisorAccounts.length]);
  const loadMoreCleaners = useCallback(async () => { if (!cleanerCursor || !modelRef.current) return; const page = await getCleanersPage({ cursor: cleanerCursor }); const current = modelRef.current; acceptModel({ ...current, cleaners: [...current.cleaners, ...page.items.filter((item) => !current.cleaners.some((loaded) => loaded.id === item.id))] }); setCleanerCursor(page.nextCursor); setCleanerTotal(page.totalCount); }, [acceptModel, cleanerCursor]);
  const loadMoreSupervisors = useCallback(async () => { if (!supervisorCursor) return; const page = await getSupervisorsPage(supervisorCursor); setSupervisorAccounts((current) => [...current, ...page.items.filter((item) => !current.some((loaded) => loaded.uid === item.uid))]); setSupervisorCursor(page.nextCursor); setSupervisorTotal(page.totalCount); }, [supervisorCursor]);
  useEffect(() => {
  ...
  const refreshAfterMutation = useCallback(async (mutation: OperationsMutation) => {
    ...
    invalidatePagedResources(resources.flatMap((resource) => resource === "alerts" ? ["/api/alerts"] : resource === "workOrders" ? ["/api/work-orders"] : resource === "cleaners" ? ["/api/cleaners"] : resource === "supervisors" ? ["/api/supervisors"] : []));
    await Promise.all([
  ...
  const dashboard = useMemo(() => ({ cameras, summary: { activeAlerts: operationsDashboard?.counts.activeAlertCount ?? allAlerts.filter((item) => activeAlert(item.status)).length, resolvedAlerts: allAlerts.filter((item) => item.status === "resolved").length, configuredCameras: operationsDashboard?.counts.cameraCount ?? cameras.length } }), [allAlerts, cameras, operationsDashboard]);
  ...
  const operationsAlertFor = (alert: Alert) => operationsAlerts.find((item) => item.id === alert.id);
  const operationsWorkFor = (id: string) => operationsWorkOrders.find((item) => item.id === id);
  async function assignAlert(alert: Alert, cleanerId: string) { const current = operationsAlertFor(alert); if (!current) throw new Error("Refresh the Alert before assigning it."); try { await assignAlertToCleaner(current.id, cleanerId); await refreshAfterMutation("alert_assignment"); } catch (reason) { actionError(reason); throw reason; } }
  async function dismissAlert(alert: Alert, reason: string) { const current = operationsAlertFor(alert); if (!current) throw new Error("Refresh the Alert before dismissing it."); try { await dismissAlertRecord(current, reason); await refreshAfterMutation("alert_dismissal"); } catch (reason) { actionError(reason); throw reason; } }
  async function createManualWork(input: { title: string; instructions: string; severity: "warning" | "critical"; assignedCleanerId: string; target: { type: "camera"; cameraId: string } | { type: "coordinate"; point: { xMeters: number; yMeters: number } } }) { try { await createManualWorkOrder(input); await refreshAfterMutation("manual_work"); } catch (reason) { actionError(reason); throw reason; } }
  async function actOnWork(id: string, action: "reassign" | "takeover" | "dismiss" | "verify" | "override", options: { cleanerId?: string; reason: string; outcome?: "passed" | "failed" | "inconclusive" }) { const work = operationsWorkFor(id); if (!work) throw new Error("Refresh the Work Order before acting on it."); try { if (action === "reassign" && options.cleanerId) await reassignWorkOrder(work, options.cleanerId, options.reason); if (action === "takeover") await takeOverWorkOrder(work, options.reason); if (action === "dismiss") await dismissWorkOrder(work, options.reason); if (action === "verify" && options.outcome) await verifyWorkOrder(work, options.outcome, options.reason); if (action === "override" && options.outcome) await overrideWorkVerification(work, options.outcome, options.reason); await refreshAfterMutation("work_decision"); } catch (reason) { actionError(reason); throw reason; } }
  async function createCleanerForSite(input: CreateCleanerInput) { try { await createCleanerAccount(input); await refreshAfterMutation("cleaner_station"); } catch (reason) { actionError(reason); throw reason; } }
  async function updateCleanerForSite(cleaner: OperationsCleaner, input: { fullName: string; phone: string; weeklySchedule: WeeklySchedule }) { try { await updateCleanerAccount(cleaner, input); await refreshAfterMutation("cleaner_update"); } catch (reason) { actionError(reason); throw reason; } }
  async function moveCleanerStation(cleaner: OperationsCleaner, point: OperationsPoint) { try { await updateCleanerStation(cleaner.id, point); await refreshAfterMutation("cleaner_station"); } catch (reason) { actionError(reason); throw reason; } }
  async function overrideCleanerAvailability(cleaner: OperationsCleaner, availabilityOverride: "none" | "unavailable") { try { await updateCleanerAccount(cleaner, { availabilityOverride }); await refreshAfterMutation("cleaner_update"); } catch (reason) { actionError(reason); throw reason; } }
  async function createSupervisorForSite(input: CreateRegularSupervisorInput) { await createRegularSupervisor(input); invalidateOperationsResources(["supervisors"]); await loadSupervisors(true); }
  async function updateSupervisorForSite(account: SupervisorManagementItem, input: { fullName?: string; phone?: string | null; status?: "active" | "inactive" }) { await updateSupervisorAccount(account, input); invalidateOperationsResources(["supervisors"]); await loadSupervisors(true); }

  const content = useMemo(() => {
    if ((page === "dashboard" || page === "admin" || page === "history") && !operationsModel) return <section className="ops-page"><header className="page-title"><div><span>LIVE OPERATIONS</span><h1>Loading the Site Map.</h1><p>The shared Site Map and operational records are still loading.</p></div></header></section>;
    if (page === "dashboard" && operationsModel) return <GeographicOperationsDashboard zones={zones} cameras={cameraRecords} alerts={allAlerts} workOrders={operationsWorkOrders} cleaners={staff} recommendations={placementRecommendations} availabilityById={availabilityById} busyZones={operationsDashboard?.busyZones} siteMap={operationsModel.siteMap} siteName={siteName} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} />;
    if (page === "admin" && operationsModel) return <TeamManagementPage viewer={{ uid: supervisor.uid, authority: supervisor.authority }} staff={staff} zones={zones} operationsCleaners={operationsModel.cleaners} cleanerTotal={cleanerTotal} cleanerHasMore={Boolean(cleanerCursor)} onLoadMoreCleaners={loadMoreCleaners} workOrders={operationsWorkOrders} siteMap={operationsModel.siteMap} supervisors={supervisorAccounts} supervisorTotal={supervisorTotal} supervisorHasMore={Boolean(supervisorCursor)} onLoadMoreSupervisors={loadMoreSupervisors} supervisorsLoading={supervisorsLoading} supervisorsError={supervisorsError} onRetrySupervisors={() => { invalidateOperationsResources(["supervisors"]); void loadSupervisors(true); }} onCreateSupervisor={createSupervisorForSite} onUpdateSupervisor={updateSupervisorForSite} onCreate={createCleanerForSite} onUpdate={updateCleanerForSite} onMoveStation={moveCleanerStation} onAvailabilityOverride={overrideCleanerAvailability} />;
    if (page === "cameras") return <CameraOperationsPage {...supervisorCameraPageAccess(capabilities)} sites={sites} zones={zones} cameras={cameraRecords} operationsCameras={operationsModel?.cameras} siteMap={operationsModel?.siteMap} workOrders={operationsWorkOrders} feeds={cameras} liveVideos={liveVideos} alerts={allAlerts} cleaners={staff} availableCleanerIds={staff.filter((cleaner) => availabilityById[cleaner.id]?.available).map((cleaner) => cleaner.id)} error={locationError} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} onWorkAction={actOnWork} onCameraPublished={async () => { await refreshAfterMutation("camera_publish"); }} onCameraRemoved={async () => { await refreshAfterMutation("camera_remove"); }} onDismissCameraWork={(workId, reason) => actOnWork(workId, "dismiss", { reason })} onCreateZone={async () => { throw new Error("Zone creation is handled by the Camera registration wizard."); }} onCreateCamera={async () => { throw new Error("Camera creation is handled by the Camera registration wizard."); }} />;
    if (page === "alerts") return <AlertManagementPage alerts={allAlerts} cameras={cameraRecords} cleaners={staff} workOrders={operationsWorkOrders} availableCleaners={staff.filter((cleaner) => availabilityById[cleaner.id]?.available)} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} onAssign={assignAlert} onDismiss={dismissAlert} onStatus={(alert, next) => changeStatus(alert, next)} />;
    if (page === "history" && operationsModel) return <WorkManagementPage siteMap={operationsModel.siteMap} workOrders={operationsWorkOrders} cleaners={staff} zones={zones} cameras={cameraRecords} alerts={allAlerts} availableCleanerIds={staff.filter((cleaner) => availabilityById[cleaner.id]?.available).map((cleaner) => cleaner.id)} onCreateWork={createManualWork} onWorkAction={actOnWork} onNavigate={(path, params) => onNavigate(path.replace(/^\//, "") as Page, params)} />;
  ...
  return <div className={`ops-shell ${drawerOpen ? "drawer-open" : ""}`}>
    ...
    <aside className="ops-sidebar" aria-label="LitterSpot navigation"><button className="ops-brand field-brand" type="button" onClick={() => go("dashboard")}><b>LS</b><div><strong>LitterSpot</strong><span>Field Station</span></div></button><nav>
      ...
      <p>System</p><button type="button" onClick={() => { location.hash = "/status"; }}>Service status</button>
