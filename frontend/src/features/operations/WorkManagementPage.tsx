...
type WorkStatus = "Unassigned" | "Assigned" | "In Progress" | "Completed" | "Cancelled";
...
const statusOptions: WorkStatus[] = ["Unassigned", "Assigned", "In Progress", "Completed", "Cancelled"];
...
function WorkCreateModal({ zones, cleaners, cameras, onClose, onCreate }: { zones: Zone[]; cleaners: Cleaner[]; cameras: Array<{ id: string; name: string }>; onClose: () => void; onCreate: (input: Omit<WorkItem, "id" | "origin" | "createdAt" | "updatedAt">) => void }) {
  ...
  function submit(event: FormEvent) {
    ...
    if (!description.trim()) { setError("Add a short work instruction before creating the work item."); return; }
    const zone = activeZones.find((item) => item.id === zoneId) ?? nearestZone;
    const camera = cameras.find((item) => item.id === cameraId);
    onCreate({ issue, description: description.trim(), priority, zoneId: zone?.id, zoneName: zone?.name ?? "Custom site point", cleanerId: cleanerId || undefined, status: cleanerId ? "Assigned" : "Unassigned", point, cameraId: camera?.id, cameraName: camera?.name });
  }
  ...
  return <div className="work-modal-scrim" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form className="work-create-modal" onSubmit={submit}>
    <header><div><span>MANUAL WORK</span><h2>Create work at any point.</h2><p>A camera link is optional. The selected pin is the primary location.</p></div><button type="button" onClick={onClose} aria-label="Close create work">×</button></header>
    ...
    {error && <p className="work-create-error" role="alert">{error}</p>}<footer><p>Manual work is shown locally in this frontend prototype. It does not create a backend record.</p><div><button type="button" onClick={onClose}>Cancel</button><button type="submit">Create work →</button></div></footer>
  </form></div>;
}
...
function WorkDetailDrawer({ work, zones, cleaners, onClose, onUpdate }: { work: WorkItem; zones: Zone[]; cleaners: Cleaner[]; onClose: () => void; onUpdate: (patch: Partial<WorkItem>) => void }) {
  ...
  return <div className="work-detail-scrim" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className="work-detail-drawer" aria-label={`Work details for ${work.id}`}>
    ...
    <section className="work-reassign"><header><div><span>ASSIGNED CLEANER</span><h3>{cleaner ? cleaner.fullName : "Unassigned"}</h3></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Cleaners" /></header><div>{activeCleaners.slice(0, 5).map((person) => <button type="button" className={person.id === work.cleanerId ? "selected" : ""} key={person.id} onClick={() => onUpdate({ cleanerId: person.id, status: work.status === "Unassigned" ? "Assigned" : work.status })}><i>{initials(person.fullName)}</i><span><b>{person.fullName}</b><small>{person.staffCode} · Available</small></span>{person.id === work.cleanerId && <em>Assigned</em>}</button>)}</div></section>
  </aside></div>;
}
...
export function WorkManagementPage({ cleaners, zones, alerts }: { cleaners: Cleaner[]; zones: Zone[]; alerts: Alert[] }) {
  ...
  const aiWork = useMemo<WorkItem[]>(() => alerts.map((alert, index) => {
    ...
    const status: WorkStatus = alert.status !== "active" ? "Completed" : index % 3 === 0 ? "Unassigned" : index % 3 === 1 ? "Assigned" : "In Progress";
    return { id: `AI-${String(alert.id).padStart(4, "0")}`, origin: "ai", issue: title(alert.kind), description: `AI detection raised from ${alert.cameraName}. Review and clear the reported condition.`, zoneId: zone?.id, zoneName: zone?.name ?? alert.zone, cleanerId: status === "Unassigned" ? undefined : assigned?.id, status, priority: alert.severity === "critical" ? "High" : "Medium", createdAt: alert.createdAt, updatedAt: alert.updatedAt, point: zone ? anchorFor(zone, activeZones.findIndex((item) => item.id === zone.id)) : undefined, cameraId: alert.cameraId, cameraName: alert.cameraName, alertId: alert.id, evidenceAvailable: alert.evidenceAvailable, detectionTime: alert.createdAt };
  ...
  return <section className="work-management-page work-list-page">
    <header className="work-list-heading"><div><span>WORK MANAGEMENT · BATU CAVES</span><h1>Keep every response moving.</h1><p>Review AI alerts and supervisor-created work from one operational queue.</p></div><button type="button" onClick={() => setShowCreate(true)}><b>+</b>Create work</button></header>
    <section className="work-metric-grid"><article><span>Total work</span><b>{allWork.length}</b></article><article className="pending"><span>Unassigned / pending</span><b>{metrics.Unassigned}</b></article><article className="assigned"><span>Assigned</span><b>{metrics.Assigned}</b></article><article className="progress"><span>In progress</span><b>{metrics["In Progress"]}</b></article><article className="complete"><span>Completed</span><b>{metrics.Completed}</b></article></section>
    ...
    <section className="work-records"><header><div><span>WORK QUEUE</span><h2>{filtered.length} work items</h2></div><small>Open linked work to supervise it in the live camera view.</small></header><div className="work-records-scroll"><table><thead><tr><th>Work ID</th><th>Issue / work type</th><th>Origin</th><th>Zone / location</th><th>Cleaner</th><th>Status</th><th>Priority</th><th>Created</th></tr></thead><tbody>{filtered.map((work) => { const cleaner = cleaners.find((item) => item.id === work.cleanerId); return <tr key={work.id} tabIndex={0} onClick={() => openWork(work)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openWork(work); }}><td><strong>{work.id}</strong></td><td><strong>{work.issue}</strong><small>{work.description}</small></td><td><span className={`work-origin ${work.origin}`}>{work.origin === "ai" ? "AI Alert" : "Manual"}</span></td><td><strong>{work.zoneName}</strong><small>{work.origin === "ai" ? work.cameraName ?? "Camera linked" : work.point ? "Map point" : "Location not set"}</small></td><td>{cleaner ? <span className="work-cleaner-cell"><i>{initials(cleaner.fullName)}</i><b>{cleaner.fullName}</b></span> : <span className="work-unassigned">Unassigned</span>}</td><td><span className={`work-record-status ${statusClass(work.status)}`}>{work.status}</span></td><td><span className={`work-priority ${work.priority.toLowerCase()}`}>{work.priority}</span></td><td>{clock(work.createdAt)}</td></tr>; })}</tbody></table>{!filtered.length && <div className="work-empty-state"><b>No work items found.</b><span>Try changing the filters or create a manual work item.</span></div>}</div></section>
