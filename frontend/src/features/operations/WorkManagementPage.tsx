...
function WorkDetailDrawer({ work, siteMap, cleaners, availableCleanerIds, onClose, onAction, readOnly = false, mediaContentUrl, onNavigate }: { work: WorkItem; siteMap: WorkSiteMap; cleaners: Cleaner[]; availableCleanerIds: Set<string>; onClose: () => void; onAction?: (action: WorkAction, options: { cleanerId?: string; reason: string; outcome?: "passed" | "failed" | "inconclusive" }) => Promise<void>; readOnly?: boolean; mediaContentUrl?: (mediaId: string) => string; onNavigate?: (path: string, params?: Record<string, string>) => void }) {
    ...
    <div className="work-proof-layout">
      <section className="work-proof-main">
        <div className={`work-proof-visual ${work.evidenceAvailable ? "submitted" : "waiting"}`}>
          {work.evidenceAvailable ? <><ProtectedWorkEvidence mediaId={work.evidenceMediaId} alt={`Completion evidence submitted for ${work.issue}`} mediaContentUrl={mediaContentUrl} /><span>COMPLETION EVIDENCE · 1 PHOTO</span><time>{clock(work.updatedAt)}</time></> : <div className="work-evidence-waiting"><strong>Waiting for evidence…</strong><small>The Cleaner must submit one completion photo before this Manual Work can be reviewed.</small></div>}
        </div>
  ...
  return <div className="work-detail-scrim" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className="work-detail-drawer" aria-label={`Work details for ${work.id}`}>
    ...
    {work.origin === "ai" && <section className="work-ai-context"><header><span>DETECTION CONTEXT</span><b>{work.alertId ? `ALERT ${work.alertId}` : "AI ALERT"}</b></header>{work.evidenceAvailable && work.evidenceMediaId && <ProtectedWorkEvidence mediaId={work.evidenceMediaId} alt="Alert evidence" mediaContentUrl={mediaContentUrl} />}<p>{work.cameraName ?? "Camera not reported"} · detected {work.detectionTime ? clock(work.detectionTime) : "—"}</p>{work.cameraId && <button type="button" onClick={() => { const params = { cameraId: work.cameraId!, zoneId: work.zoneId ?? "all", from: "work" }; if (onNavigate) onNavigate("/cameras", params); else location.hash = `/cameras?${new URLSearchParams(params)}`; }}>Open camera context →</button>}</section>}
...
export function WorkManagementPage({ siteMap, cleaners, zones, cameras, alerts, workOrders, availableCleanerIds = [], onCreateV2Work, onV2Action, readOnly = false, showPermissionNotice = true, mediaContentUrl, onNavigate }: { siteMap: WorkSiteMap; cleaners: Cleaner[]; zones: Zone[]; cameras: CameraRecord[]; alerts: Alert[]; workOrders?: V2WorkOrder[]; availableCleanerIds?: string[]; onCreateV2Work?: (input: ManualWorkInput) => Promise<void>; onV2Action?: (workId: string, action: WorkAction, options: { cleanerId?: string; reason: string; outcome?: "passed" | "failed" | "inconclusive" }) => Promise<void>; readOnly?: boolean; showPermissionNotice?: boolean; mediaContentUrl?: (mediaId: string) => string; onNavigate?: (path: string, params?: Record<string, string>) => void }) {
  ...
  function openWork(work: WorkItem) {
    ...
    if (onNavigate) onNavigate("/cameras", Object.fromEntries(query));
    else location.hash = `/cameras?${query.toString()}`;
  }
  ...
  return <section className="work-management-page work-list-page">
    ...
    {selected && <WorkDetailDrawer readOnly={readOnly} work={selected} siteMap={siteMap} cleaners={cleaners} availableCleanerIds={availableCleanerSet} onClose={() => setSelectedId(undefined)} onAction={onV2Action ? (action, options) => onV2Action(selected.id, action, options) : undefined} mediaContentUrl={mediaContentUrl} onNavigate={onNavigate} />}
  </section>;
}
