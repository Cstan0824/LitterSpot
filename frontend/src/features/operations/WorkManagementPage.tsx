...
export function WorkManagementPage({ siteMap, cleaners, zones, cameras, alerts, workOrders, loadPage = getV2WorkOrdersPage, availableCleanerIds = [], onCreateV2Work, onV2Action, readOnly = false, showPermissionNotice = true, mediaContentUrl, onNavigate }: { siteMap: WorkSiteMap; cleaners: Cleaner[]; zones: Zone[]; cameras: CameraRecord[]; alerts: Alert[]; workOrders?: V2WorkOrder[]; loadPage?: (input: { limit?: number; cursor?: string; status?: string; zoneId?: string; cameraId?: string; cleanerId?: string; origin?: string }, signal?: AbortSignal) => Promise<V2WorkOrderPage>; availableCleanerIds?: string[]; onCreateV2Work?: (input: ManualWorkInput) => Promise<void>; onV2Action?: (workId: string, action: WorkAction, options: { cleanerId?: string; reason: string; outcome?: "passed" | "failed" | "inconclusive" }) => Promise<void>; readOnly?: boolean; showPermissionNotice?: boolean; mediaContentUrl?: (mediaId: string) => string; onNavigate?: (path: string, params?: Record<string, string>) => void }) {
  ...
  const backendFilters = useMemo(() => ({ status: statusFilter === "All" ? "all" : statusFilter.toLowerCase().replaceAll(" ", "_"), ...(zoneFilter !== "all" && zoneFilter !== "unzoned" ? { zoneId: zoneFilter } : {}), ...(originFilter !== "all" ? { origin: originFilter === "ai" ? "alert" : "manual" } : {}) }), [originFilter, statusFilter, zoneFilter]);
  useEffect(() => {
  ...
  useEffect(() => {
    const syncQuery = () => { const query = new URLSearchParams(location.hash.split("?")[1] ?? ""); setSelectedId(query.get("workId") ?? undefined); setZoneFilter(query.get("zoneId") ?? "all"); };
  ...
  function openWork(work: WorkItem) {
    const params: Record<string, string> = { workId: work.id }; if (zoneFilter !== "all") params.zoneId = zoneFilter;
    if (onNavigate) onNavigate("/history", params); else location.hash = `/history?${new URLSearchParams(params)}`;
  }
  function closeWork() { setDetailError(""); const params: Record<string, string> = {}; if (zoneFilter !== "all") params.zoneId = zoneFilter; if (onNavigate) onNavigate("/history", params); else location.hash = `/history${Object.keys(params).length ? `?${new URLSearchParams(params)}` : ""}`; }
