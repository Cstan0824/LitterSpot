...
export type WorkItem = {
  ...
  point?: Point; cameraId?: string; cameraName?: string; alertId?: string; managementMode?: string; verificationOutcome?: string | null; evidenceAvailable?: boolean; detectionTime?: string;
};
...
export function WorkManagementPage({ cleaners, zones, alerts, workOrders, availableCleanerIds = [], onCreateV2Work, onV2Action, readOnly = false }: { cleaners: Cleaner[]; zones: Zone[]; alerts: Alert[]; workOrders?: V2WorkOrder[]; availableCleanerIds?: string[]; onCreateV2Work?: (input: { title: string; instructions: string; severity: "warning" | "critical"; assignedCleanerId: string; target: { type: "camera"; cameraId: string } }) => Promise<void>; onV2Action?: (workId: string, action: WorkAction, options: { cleanerId?: string; reason: string; outcome?: "passed" | "failed" | "inconclusive" }) => Promise<void>; readOnly?: boolean }) {
  ...
  function openWork(work: WorkItem) { setSelectedId(work.id); }

  return <section className="work-management-page work-list-page">
    <header className="work-list-heading"><div><span>WORK ORDER MANAGEMENT</span><h1>Keep every response moving.</h1><p>Review AI Alerts and supervisor-created Manual Work Orders from one operational queue.</p></div>{!readOnly && <button type="button" onClick={() => setShowCreate(true)}><b>+</b>Create Work Order</button>}</header>{readOnly && <p className="profile-feedback">Read-only V2 Work data. Work actions begin in Phase 12.3.</p>}
