...
import type { V2OperationsReadModel } from "../../services/v2/operations";
...
export function GeographicOperationsDashboard({ zones, cameras, alerts, cleaners, recommendations, availabilityById = {}, busyZones, siteMap, siteName = "Active site" }: {
  zones: Zone[];
  ...
  return <section className="atlas-dashboard">
    ...
    <section className="atlas-overview-grid">
      ...
      <OverviewCard title="Evidence exceptions" action="Inspect cameras" onOpen={() => navigate("/cameras")}><div className="atlas-exception-list">{exceptions.map((camera) => <button key={camera.id} onClick={() => navigate("/cameras", { cameraId: camera.id, from: "dashboard" })}><i className={camera.availability === "unavailable" ? "action" : "review"} /><span><strong>{camera.name}</strong><small>{camera.zoneName} · {camera.availability === "unavailable" ? "Camera offline" : `${camera.registrationStatus} registration`}</small></span></button>)}{!exceptions.length && <p className="atlas-empty">All current evidence sources are healthy.</p>}</div></OverviewCard>
