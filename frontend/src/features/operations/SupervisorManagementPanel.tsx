...
export function SupervisorManagementPanel({ viewer, supervisors, loading, error, createRequest, onRetry, onCreate, onUpdate, readOnly = false }: { viewer: Viewer; supervisors: SupervisorListItem[]; loading: boolean; error?: string; createRequest: number; onRetry: () => void; onCreate: (input: CreateRegularSupervisorInput) => Promise<void>; onUpdate: (supervisor: SupervisorManagementItem, input: UpdateInput) => Promise<void>; readOnly?: boolean }) {
  ...
  const rootAccounts = useMemo(() => shown.filter((supervisor) => supervisor.authority === "root"), [shown]);
  ...
  return <div className="team-supervisor-panel">
    ...
    <section className="team-roster team-supervisor-roster"><header><div><span>{root ? "SUPERVISOR ACCOUNTS" : "ACTIVE SITE TEAM"}</span><h2>{root ? "Supervisor directory" : "Site Supervisors"}</h2></div><p>{root ? readOnly ? "Supervisor identity, authority, and account access for this Site." : "Select a Regular Supervisor to edit their profile or account access." : "Names and authority are visible for coordination. Private account details are hidden."}</p></header><div className="team-supervisor-table"><div className={`team-supervisor-row label ${root ? "root-view" : "regular-view"}`}><span>Supervisor</span>{root && <span>Email</span>}<span>Authority</span>{root && <span>Account status</span>}</div>{root ? <>{rootAccounts.length > 0 && <div className="team-supervisor-group-divider root-view"><span>ROOT SUPERVISOR <small>Site authority</small></span></div>}{rootAccounts.map(supervisorRow)}{regularAccounts.length > 0 && <div className="team-supervisor-group-divider root-view"><span>REGULAR SUPERVISOR ACCOUNTS <small>Created and managed by Root Supervisor</small></span></div>}{regularAccounts.map(supervisorRow)}</> : shown.map(supervisorRow)}{!shown.length && <p className="team-empty">{query ? "No Supervisors match this search." : "No Supervisor accounts match this filter."}</p>}</div></section>
    {!readOnly && creating && <CreateSupervisorModal onClose={() => setCreating(false)} onCreate={create} />}
    {!readOnly && selected && <SupervisorDetailModal supervisor={selected} onClose={() => setSelectedUid("")} onUpdate={update} />}
  </div>;
}
