import type { OperationsAlert } from "../../services/api/operations";

const uuidLike = (value: string) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value);

export function alertZoneDisplayName(alert: Pick<OperationsAlert, "zoneId" | "zoneNameSnapshot">, zoneNames: Map<string, string>) {
