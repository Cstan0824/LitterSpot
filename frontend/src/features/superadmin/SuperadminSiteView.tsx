...
import { adaptV2Operations } from "../operations/OperationsConsole";
...
function emptyDashboard(view: V2SuperadminOperationsView): V2Dashboard {
...
function InsightsPage({ siteId }: { siteId: string }) {
  ...
  useEffect(() => { const controller = new AbortController(); void getV2SuperadminAnalytics(siteId, {}, controller.signal).then(setData).catch((reason) => { if (!controller.signal.aborted) setError(errorCopy(reason)); }); return () => controller.abort(); }, [siteId]);
...
export function SuperadminSiteView({ siteId, page, profile, onNavigate, onExit, onLogout }: { siteId: string; page: SuperadminSiteViewPage; profile: Profile; onNavigate: Navigate; onExit: () => void; onLogout: () => void }) {
  const [view, setView] = useState<V2SuperadminOperationsView>();
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(true);
  const load = useCallback(async () => { setLoading(true); setError(""); try { setView(await getV2SuperadminOperationsView(siteId)); } catch (reason) { setError(errorCopy(reason)); } finally { setLoading(false); } }, [siteId]);
