...
export function SuperadminApp({ profile, onLogout }: { profile: Profile; onLogout: () => void }) {
  ...
  return <div className="sa-shell"><header className="field-nav-head sa-platform-nav"><button className="field-nav-logo" type="button" onClick={() => navigate({ kind: "sites" })}>LitterSpot</button><nav className="field-primary-nav" aria-label="Superadmin navigation"><button type="button" className={currentSection === "sites" ? "active" : ""} aria-current={currentSection === "sites" ? "page" : undefined} onClick={() => navigate({ kind: "sites" })}><span className="field-nav-icon"><Icon name="sites" /></span><span>Sites</span></button><button type="button" className={currentSection === "audit" ? "active" : ""} aria-current={currentSection === "audit" ? "page" : undefined} onClick={() => navigate({ kind: "audit" })}><span className="field-nav-icon"><Icon name="audit" /></span><span>Audit history</span></button></nav><button className="field-nav-user" type="button" onClick={() => setDialog("account")} aria-label={`Open account controls for ${profile.displayName}`}>{initials(profile.displayName)}</button></header>{content}
