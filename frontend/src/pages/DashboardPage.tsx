...
type Health = { status: "ok" | "degraded"; aiService?: { status?: string; modelReady?: boolean; modelVersion?: string; modelArtifact?: string; device?: string; reason?: string | null } };
...
export function DashboardPage({ onOpenPlayground }: DashboardPageProps) {
  ...
  return <main className="app-shell dashboard-shell">
    <header className="dashboard-header"><div><p className="eyebrow">LITTERSPOT / OPERATIONS</p><h1>Bin monitoring<br /><em>at a glance.</em></h1></div><button className="primary" onClick={onOpenPlayground}>Open state playground <span>→</span></button></header>
    ...
    <section className="dashboard-grid">
      <article className="card overview-card"><p className="eyebrow">System overview</p><h2>Connection proof</h2><p className="card-copy">This dashboard is intentionally a simple operational sample. Its health check travels from the React browser through the Node API and then to the private Python service.</p><div className="pipeline"><span>React</span><i>→</i><span>Node API</span><i>→</i><span>FastAPI</span><i>→</i><span>MobileNetV3</span></div><button className="text-button" onClick={onOpenPlayground}>Classify a bin region →</button></article>
      ...
      <article className="card model-card"><p className="eyebrow">Loaded model</p><h2>{health?.aiService?.modelVersion ?? "No model reported"}</h2><p className="model-path">{health?.aiService?.modelArtifact ?? health?.aiService?.reason ?? "The Node API has not returned model information yet."}</p><span className={online ? "tag ready-tag" : "tag"}>{online ? "Ready for playground" : "Awaiting service"}</span></article>
    </section>
    <section className="card next-card"><div><p className="eyebrow">Next action</p><h2>Validate the end-to-end state flow</h2><p>Open the playground, upload a cropped bin region from a fixed CCTV view, and inspect its normal or overflow probability.</p></div><button className="primary" onClick={onOpenPlayground}>Go to playground <span>→</span></button></section>
  </main>;
}
