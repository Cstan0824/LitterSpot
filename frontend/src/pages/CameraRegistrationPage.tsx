...
export function CameraRegistrationPage({ canCreateCamera = true }: { canCreateCamera?: boolean }) {
  ...
  async function startRegistration(event: FormEvent) {
    ...
    if (selected) return;
    if (!canCreateCamera) {
      setError("Select an existing Camera to open its registration.");
      return;
    }
