...
export function SiteAdministrationPage({ siteName, readOnly = false }: { siteName: string; readOnly?: boolean }) {
  ...
  const load = async (signal?: AbortSignal) => {
    ...
    try {
      const map = await getActiveSiteMap(signal);
      setActiveMap(map);
  ...
  useEffect(() => { const controller = new AbortController(); void load(controller.signal); return () => controller.abort(); }, [readOnly]);
  useEffect(() => {
          ...
          <div className="site-dimension-fields">
            ...
            <div className="site-grid-field">
              ...
              <div className="site-grid-guidance" id="site-grid-guidance">
                <span>{settings.gridIntervalMode === "recommended" ? "Recommended automatically" : recommendedGridInterval === null ? "Enter a valid width to calculate a recommendation" : `Custom value · ${number.format(recommendedGridInterval)} m recommended`}</span>
