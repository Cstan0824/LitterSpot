...
export const getActiveSiteMap = (signal?: AbortSignal) => v2Request<{ map: ActiveSiteMap }>("/api/site-map", { signal }).then((response) => response.map);
export const getSiteMapDraft = (signal?: AbortSignal) => v2Request<Parameters<typeof normalizeDraft>[0]>("/api/site-map/draft", { signal }).then(normalizeDraft);
export const getRetiredSiteMapZones = (signal?: AbortSignal) => v2Request<{ zones: SiteMapZone[] }>("/api/site-map/retired-zones", { signal }).then((response) => response.zones);
export const startSiteMapDraft = () => v2Request<{ draft: SiteMapDraft }>("/api/site-map/draft/start", { method: "POST", json: {} }).then((response) => normalizeDraft({ draft: response.draft }));
export const saveSiteMapDraft = (input: SiteMapDraftSave) => v2Request<Parameters<typeof normalizeDraft>[0]>("/api/site-map/draft", { method: "POST", json: input }).then(normalizeDraft);
export const validateSiteMapDraft = () => v2Request<SiteMapValidation>("/api/site-map/draft/validate", { method: "POST", json: {} });
export const publishSiteMapDraft = () => v2Request<{ map: ActiveSiteMap }>("/api/site-map/draft/publish", { method: "POST", json: {} }).then((response) => response.map);
export const discardSiteMapDraft = () => v2Request<void>("/api/site-map/draft", { method: "DELETE" });
export const uploadSiteMapBackground = (image: File) => { const body = new FormData(); body.append("image", image, image.name); return v2Request<{ background: SiteMapBackground }>("/api/site-map/background", { method: "POST", body }).then((response) => response.background); };
export const getSiteMapAuditEvents = (signal?: AbortSignal) => v2Request<{ events: SiteMapAuditEvent[] }>("/api/operations/v2/audit-events", { signal }).then((response) => response.events);
export const changeSiteMapCameraPlacement = (input: { cameraId: string; point: SiteMapPoint; mode: "map_position_correction" | "physical_camera_move"; reason: string; expectedCameraRevision: number; expectedMapRevisionId: string }) => v2Request<{ mode: typeof input.mode; status: "published" | "registration_required"; cameraId: string; zoneId: string; point: SiteMapPoint; mapRevisionId?: string; cameraRevision?: number; draft?: import("./operations").V2CameraDraft }>(`/api/site-map/camera-placements/${encodeURIComponent(input.cameraId)}`, { method: "POST", json: { point: input.point, mode: input.mode, reason: input.reason, expectedCameraRevision: input.expectedCameraRevision, expectedMapRevisionId: input.expectedMapRevisionId, confirmation: true } });
