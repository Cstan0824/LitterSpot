...
export const implementBinPlacement = (zoneId: string, snapshotCalculatedAt: string, note?: string) =>
  v2Request<{ intervention: BinPlacementIntervention }>(`/api/bin-placement/v2/zones/${encodeURIComponent(zoneId)}/implement`, {
    method: "POST",
    json: { snapshotCalculatedAt, ...(note?.trim() ? { note: note.trim() } : {}) },
  });

export const getBinPlacementInterventions = (signal?: AbortSignal) =>
  v2Request<{ interventions: BinPlacementIntervention[] }>("/api/bin-placement/v2/interventions", { signal });

export const getBinPlacementComparison = (interventionId: string, days: number, signal?: AbortSignal) =>
  v2Request<{ comparison: BinPlacementComparison }>(`/api/bin-placement/v2/interventions/${encodeURIComponent(interventionId)}/comparison?days=${encodeURIComponent(days)}`, { signal });
