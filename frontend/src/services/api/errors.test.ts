...
import { ApiError, loginErrorMessage, parseApiError, sessionErrorCopy } from "./errors";
...
describe("structured API errors", () => {
  ...
  it("separates inactive access, forbidden access, and backend unavailability", () => {
    expect(sessionErrorCopy(new ApiError({ status: 403, message: "Application access is inactive." })).title).toBe("Application access is inactive");
    expect(sessionErrorCopy(new ApiError({ status: 403, message: "Supervisor access is required." })).title).toBe("Application access is forbidden");
    expect(sessionErrorCopy(new ApiError({ status: 503, message: "Service unavailable." })).title).toBe("Backend unavailable");
  });
});
