...
The full product workflow remains documented in
[`LITTERSPOT_SYSTEM_PRD.md`](LITTERSPOT_SYSTEM_PRD.md). This implementation plan
ends at a versioned `ConfirmedFlag` contract so the later incident and task
modules can consume detections without depending on VLM internals.
