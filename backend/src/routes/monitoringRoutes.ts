...
import { claimMonitoringSession, heartbeatMonitoringSession } from "../services/monitoringLease.js";
import multer from "multer";
import { z } from "zod";
import { flushMinuteBuckets, markOfflineCameras, processLiveSample, startMonitoringEpisode } from "../services/liveMonitoringService.js";
import { evaluateAlertsForCamera } from "../services/alertService.js";
import { recordCameraVerificationObservation } from "../services/workOrderService.js";
import { endMonitoringEpisode } from "../services/liveMonitoringService.js";
...
monitoringRoutes.post("/sessions/claim", async (req, res) => {
...
monitoringRoutes.post("/sessions/:sessionId/heartbeat", async (req, res) => {
  const siteId = String(req.supervisor.siteId ?? req.body.siteId ?? "");
...
monitoringRoutes.post("/sessions/:sessionId/release", async (req, res) => {
  const siteId = String(req.supervisor.siteId ?? req.body.siteId ?? "");
  const token = String(req.header("x-monitoring-token") ?? req.body.leaseToken ?? "");
  const current = runtimeSession(siteId);
...
monitoringRoutes.post("/sessions/:sessionId/cameras/:cameraId/start", async (req, res) => {
  const token = String(req.header("x-monitoring-token") ?? req.body.leaseToken ?? "");
...
monitoringRoutes.post("/sessions/:sessionId/cameras/:cameraId/samples", frameUpload.single("frame"), async (req, res) => {
  ...
  const alertEvaluation = result.operationalSample ? await evaluateAlertsForCamera(String(req.supervisor.siteId), String(req.params.cameraId), input.episodeId) : [];
  const verificationsApplied = result.operationalSample ? await recordCameraVerificationObservation(String(req.supervisor.siteId), String(req.params.cameraId), result.observation) : 0;
...
monitoringRoutes.post("/sessions/:sessionId/cameras/:cameraId/stop", async (req, res) => {
...
monitoringRoutes.post("/offline-sweep", async (req, res) => {
...
monitoringRoutes.post("/minute-flush", async (req, res) => res.json({ bucketsFlushed: await flushMinuteBuckets(String(req.supervisor.siteId), new Date()) }));
