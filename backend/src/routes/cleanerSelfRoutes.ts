...
async function cleanerTransition(req: Request, res: Response, status: "in_progress" | "awaiting_review") {
  ...
  return res.json({ workOrder: await transitionV2WorkOrder(String(req.authUser.siteId), String(req.params.workOrderId), status, { ...req.authUser, type: "cleaner", cleanerId: req.cleaner!.cleanerId } as any, input, req.requestId) });
}
