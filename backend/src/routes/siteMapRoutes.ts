...
siteMapRoutes.post("/draft", async (req, res) => {
...
siteMapRoutes.post("/draft/validate", async (req, res) => res.json(await validateV2MapDraft(String(req.supervisor.siteId))));
siteMapRoutes.post("/draft/publish", async (req, res) => res.json({ map: await publishV2MapDraft(String(req.supervisor.siteId), req.authUser.uid) }));
siteMapRoutes.delete("/draft", async (req, res) => {
  if (req.authUser.authority !== "root") return res.status(403).json({ error: "Root Supervisor access is required.", requestId: req.requestId });
