...
import { flushMinuteBuckets, inspectLiveMemory, markOfflineCameras, resetLiveMemory, setLiveInferenceForTests } from "./services/liveMonitoringService.js";
...
run("live monitoring", () => {
  ...
  it("claims one owner, starts an Episode, and processes a sample without storing the frame", async () => {
    ...
    expect(inspectLiveMemory(cameraId).temporalKeys).toHaveLength(3);
    ...
    expect(await flushMinuteBuckets(siteId, new Date())).toBe(0);
    expect(await flushMinuteBuckets(siteId, new Date(Date.now() + 60000))).toBe(1);
  ...
  it("combines full and overflow into one escalating bin-service Alert", async () => {
    ...
    await flushMinuteBuckets(siteId,new Date(Date.now()+120000));
