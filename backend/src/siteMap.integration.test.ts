...
run("Site Map workflow", () => {
  ...
  it("publishes map corrections and protects Physical Camera Move drafts", async () => {
    ...
    const destinationZoneId = `move-zone-${suffix}`;
    await startDraft();
    const destinationZone = { zoneId: destinationZoneId, zoneNameSnapshot: "Move Destination", polygon: [{ xMeters: 50, yMeters: 50 }, { xMeters: 90, yMeters: 50 }, { xMeters: 90, yMeters: 90 }, { xMeters: 50, yMeters: 90 }] };
    const zoneSave = await request(app).post("/api/site-map/draft").set("Authorization", `Bearer ${token}`).send({ baseRevisionId: map.activeRevisionId, expectedRevision: 1, widthMeters: map.revision.widthMeters, heightMeters: map.revision.heightMeters, gridSizeMeters: map.revision.gridSizeMeters, backgroundMediaId: map.revision.backgroundMediaId ?? null, backgroundTransform: map.revision.backgroundTransform ?? null, zones: [...map.zones.map((item) => ({ zoneId: item.zoneId, zoneNameSnapshot: item.zoneNameSnapshot, polygon: item.polygon })), destinationZone], cameraPlacements: map.cameraPlacements.map((placement) => ({ id: placement.id, point: placement.point })), cleanerStations: map.cleanerStations.map((station) => ({ id: station.id, point: station.point })) });
    expect(zoneSave.status).toBe(200);
    expect((await request(app).post("/api/site-map/draft/validate").set("Authorization", `Bearer ${token}`)).body.valid).toBe(true);
    expect((await request(app).post("/api/site-map/draft/publish").set("Authorization", `Bearer ${token}`)).status).toBe(200);
    ...
    await startDraft();
    const zone = map.zones.find((item) => item.zoneId === `zone-a-${suffix}`)!;
    ...
    const waitingAlertId = `move-waiting-alert-${suffix}`;
    const assignedAlertId = `move-assigned-alert-${suffix}`;
    const workOrderId = `move-work-${suffix}`;
    const now = Timestamp.now();
    await firestore.collection("alerts").doc(waitingAlertId).set({ schemaVersion: 2, alertId: waitingAlertId, siteId, mapRevisionId: map.activeRevisionId, zoneId: zone.zoneId, zoneNameSnapshot: zone.zoneNameSnapshot, cameraId, cameraNameSnapshot: "Move Camera", registrationRevisionId: "registration-1", issueType: "floor_litter", observedCondition: "litter", status: "waiting_for_cleaner", severity: "warning", activeWorkOrderId: null, managementMode: "orchestrated", createdAt: now, updatedAt: now, revision: 1 });
    await firestore.collection("alerts").doc(assignedAlertId).set({ schemaVersion: 2, alertId: assignedAlertId, siteId, mapRevisionId: map.activeRevisionId, zoneId: zone.zoneId, zoneNameSnapshot: zone.zoneNameSnapshot, cameraId, cameraNameSnapshot: "Move Camera", registrationRevisionId: "registration-1", issueType: "bin_service", observedCondition: "full", status: "assigned", severity: "warning", activeWorkOrderId: workOrderId, managementMode: "orchestrated", createdAt: now, updatedAt: now, revision: 1 });
    await firestore.collection("workOrders").doc(workOrderId).set({ schemaVersion: 2, workOrderId, siteId, origin: "alert", alertId: assignedAlertId, managementMode: "orchestrated", status: "assigned", severity: "warning", issueType: "bin_service", title: "Service the registered bin", instructions: "Service the registered bin", target: { type: "camera", mapRevisionId: map.activeRevisionId, zoneId: zone.zoneId, zoneNameSnapshot: zone.zoneNameSnapshot, point: seedPlacement, cameraId, cameraNameSnapshot: "Move Camera" }, mapRevisionId: map.activeRevisionId, zoneId: zone.zoneId, cameraId, assignedCleanerId: cleanerId, cleanerNameSnapshot: "Map Cleaner", assignedAt: now, assignedBy: { type: "orchestrator", serviceId: "test", displayNameSnapshot: "Test" }, createdAt: now, updatedAt: now, revision: 1 });
    await firestore.collection("cleaners").doc(cleanerId).update({ activeWorkOrderId: workOrderId, activeWorkAssignedAt: now });
    ...
    expect(outsideZone.body.details.code).toBe("camera_position_correction_zone_change");
    ...
    const correctedMapId = correction.body.placement.mapRevisionId;
    expect((await firestore.collection("alerts").doc(waitingAlertId).get()).data()).toMatchObject({ status: "waiting_for_cleaner", mapRevisionId: correctedMapId, zoneId: zone.zoneId });
    expect((await firestore.collection("alerts").doc(assignedAlertId).get()).data()).toMatchObject({ status: "assigned", mapRevisionId: correctedMapId, zoneId: zone.zoneId });
    expect((await firestore.collection("workOrders").doc(workOrderId).get()).data()).toMatchObject({ status: "assigned", mapRevisionId: correctedMapId, zoneId: zone.zoneId, target: { point: { xMeters: 15, yMeters: 15 }, mapRevisionId: correctedMapId } });
    expect((await firestore.collection("notifications").where("recipientUid", "==", `auth-${cleanerId}`).get()).docs.some((entry) => entry.data().type === "work_location_corrected")).toBe(true);
    const registrationBefore = (await firestore.collection("cameras").doc(cameraId).get()).data()?.activeRegistrationRevisionId;

    const physicalBody = { point: { xMeters: 60, yMeters: 60 }, mode: "physical_camera_move", reason: "The Camera was installed at a new position.", expectedCameraRevision: 2, expectedMapRevisionId: correction.body.placement.mapRevisionId, confirmation: true };
    ...
    expect((await request(app).get(`/api/camera-creation/cameras/${cameraId}/draft`).set("Authorization", `Bearer ${token}`)).body.draft).toMatchObject({ id: draftId, kind: "physical_move" });
    expect((await request(app).get(`/api/camera-creation/cameras/${cameraId}/draft`).set("Authorization", `Bearer ${regularToken}`)).status).toBe(403);
    ...
    expect((await firestore.collection("siteMapRevisions").doc(movedMapId).collection("cameraPlacements").doc(cameraId).get()).data()).toMatchObject({ point: { xMeters: 60, yMeters: 60 }, zoneId: destinationZoneId, zoneNameSnapshot: "Move Destination" });
    expect((await firestore.collection("alerts").doc(waitingAlertId).get()).data()).toMatchObject({ status: "dismissed", dismissReason: "camera_physically_moved" });
    expect((await firestore.collection("alerts").doc(assignedAlertId).get()).data()).toMatchObject({ status: "dismissed", dismissReason: "camera_physically_moved", activeWorkOrderId: null });
    expect((await firestore.collection("workOrders").doc(workOrderId).get()).data()).toMatchObject({ status: "dismissed", dismissReason: "camera_physically_moved" });
    expect((await firestore.collection("cleaners").doc(cleanerId).get()).data()?.activeWorkOrderId).toBeNull();
    expect((await firestore.collection("orchestratorOutbox").where("siteId", "==", siteId).get()).docs.some((entry) => entry.data().triggerType === "cleaner_released_by_camera_move")).toBe(true);
    const moveAudits = await firestore.collection("auditEvents").where("siteId", "==", siteId).where("action", "==", "camera_physically_moved").get();
    expect(moveAudits.docs.at(-1)?.data()).toMatchObject({ actorUid: uid, actorAuthority: "root", reason: "The Camera was installed at a new position.", before: { placement: { point: { xMeters: 15, yMeters: 15 } } }, after: { placement: { point: { xMeters: 60, yMeters: 60 } } } });
    const finalCamera = movedCamera.data()!;
    const provisionalZoneId = `physical-provisional-zone-${suffix}`;
    const provisional = await request(app).post(`/api/site-map/camera-placements/${cameraId}`).set("Authorization", `Bearer ${token}`).send({ point: { xMeters: 20, yMeters: 70 }, mode: "physical_camera_move", reason: "Testing a provisional destination Zone.", expectedCameraRevision: finalCamera.revision, expectedMapRevisionId: movedMapId, confirmation: true, provisionalZone: { zoneId: provisionalZoneId, zoneNameSnapshot: "Physical Move Court", polygon: [{ xMeters: 5, yMeters: 55 }, { xMeters: 40, yMeters: 55 }, { xMeters: 40, yMeters: 90 }, { xMeters: 5, yMeters: 90 }] } });
    expect(provisional.status).toBe(200);
    expect(provisional.body.placement.draft).toMatchObject({ kind: "physical_move", placement: { zoneId: provisionalZoneId }, provisionalZone: { zoneId: provisionalZoneId, zoneNameSnapshot: "Physical Move Court" } });
    const provisionalDraftId = provisional.body.placement.draft.id;
    expect((await request(app).post(`/api/camera-creation/drafts/${provisionalDraftId}/reference`).set("Authorization", `Bearer ${token}`).attach("image", png, { filename: "provisional-reference.png", contentType: "image/png" })).status).toBe(201);
    expect((await request(app).put(`/api/camera-creation/drafts/${provisionalDraftId}/registration`).set("Authorization", `Bearer ${token}`).send({ sourceWidth: 640, sourceHeight: 480, walkableFloorPolygon: [{ x: 0.05, y: 0.55 }, { x: 0.95, y: 0.55 }, { x: 0.95, y: 0.95 }, { x: 0.05, y: 0.95 }], bins: [] })).status).toBe(200);
    expect((await request(app).post(`/api/camera-creation/drafts/${provisionalDraftId}/validate`).set("Authorization", `Bearer ${token}`).send({})).body.valid).toBe(true);
    expect((await request(app).post(`/api/camera-creation/drafts/${provisionalDraftId}/publish`).set("Authorization", `Bearer ${token}`).send({})).status).toBe(200);
    expect((await firestore.collection("zones").doc(provisionalZoneId).get()).data()).toMatchObject({ lifecycleStatus: "active", name: "Physical Move Court" });
    const provisionalMapId = (await firestore.collection("sites").doc(siteId).get()).data()?.activeMapRevisionId;
    expect((await firestore.collection("siteMapRevisions").doc(provisionalMapId).collection("cameraPlacements").doc(cameraId).get()).data()).toMatchObject({ point: { xMeters: 20, yMeters: 70 }, zoneId: provisionalZoneId });
    const latestCamera = (await firestore.collection("cameras").doc(cameraId).get()).data()!;
    const cancellable = await request(app).post(`/api/site-map/camera-placements/${cameraId}`).set("Authorization", `Bearer ${token}`).send({ point: { xMeters: 25, yMeters: 25 }, mode: "physical_camera_move", reason: "Testing safe cancellation.", expectedCameraRevision: latestCamera.revision, expectedMapRevisionId: provisionalMapId, confirmation: true });
