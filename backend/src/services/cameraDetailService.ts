...
import { listOrchestratorRuns } from "./orchestratorService.js";
...
export async function getCameraDetail(siteId: string, cameraId: string) {
  ...
  const workOrders = workDocuments
    ...
    .map((document) => serializeFirestore({ id: document.id, ...document.data() }));
  ...
  const [runs, auditEvents] = await Promise.all([
    listOrchestratorRuns(siteId, 100),
    listAuditEvents({ siteId, limit: 100 }),
  ]);
