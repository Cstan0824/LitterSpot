...
export async function presentCleaner(cleanerId: string, expectedSiteId: string, at = new Date()) {
...
export async function listCleaners(siteId: string, status: "active" | "inactive" | "all" = "all") {
  return (await listCleanersPage(siteId, { status, limit: 100 })).items;
}

export async function listCleanersPage(siteId: string, input: { status: "active" | "inactive" | "all"; limit: number; cursor?: string }) {
  ...
  return { ...page, items: await Promise.all(page.items.map((id) => presentCleaner(id, siteId))) };
}
...
export async function createCleaner(input: {
  ...
  if (started.replay) return presentCleaner(started.profileId, input.siteId);
  ...
  try {
    ...
    await publishCleanerStation({ siteId: input.siteId, cleanerId: cleanerRef.id, point: input.stationPoint, actorUid: actor.uid, actorAuthority: actor.authority ?? "regular", actorName: actor.displayName, requestId });
    ...
    await writeAuditEvent({ actor, siteId: input.siteId, siteNameSnapshot: String(site.data()?.name), action: "cleaner_created", resourceType: "Cleaner", resourceId: cleanerRef.id, outcome: "succeeded", after: { staffCode: code, status: "active" }, requestId });
    return presentCleaner(cleanerRef.id, input.siteId);
  } catch (error) {
...
export async function updateCleaner(input: { siteId: string; cleanerId: string; expectedRevision: number; fullName?: string; phone?: string; notes?: string | null; profileMediaId?: string | null; weeklySchedule?: WeeklySchedule; availabilityOverride?: "none" | "unavailable"; status?: "active" | "inactive" }, actor: AuditActor & { uid: string }, requestId: string) {
  ...
  if (nextStatus === "active" && (input.weeklySchedule !== undefined || input.availabilityOverride === "none" || input.status === "active")) {
    await enqueueImmediateAssignmentTrigger(input.siteId, "cleaner_availability_changed", `cleaner:${input.cleanerId}:${Date.now()}`);
  }
  return presentCleaner(input.cleanerId, input.siteId);
}
