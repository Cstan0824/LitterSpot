...
import { shouldAcceptSample } from "./monitoringLease.js";
...
export type EvidenceCandidate = { confidence: number; frame: Buffer; mimeType: string; observation: LiveObservation };
...
const temporal = new Map<string, LiveObservation[]>();
...
export function setLiveInferenceForTests(replacement: typeof inferFrame | null) { inference = replacement ?? inferFrame; }
export function resetLiveMemory(cameraId?: string, preserveMinute = false) {
...
export function inspectLiveMemory(cameraId: string) {
...
export function getLiveIssueState(siteId: string, cameraId: string, issueType: "floor_litter" | "floor_spill" | "bin_service") {
...
export function clearEvidenceCandidate(siteId: string, cameraId: string, issueType: string) { evidence.delete(`${siteId}:${cameraId}:${issueType}`); }
...
export async function flushMinuteBuckets(siteId: string, before = new Date()) {
  const work = async () => {
