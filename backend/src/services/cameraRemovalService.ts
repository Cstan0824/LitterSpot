...
import { endMonitoringEpisode } from "./liveMonitoringService.js";
import { enqueueOrchestratorTriggerInTransaction } from "./orchestratorTriggers.js";
import { canonicalHash, operationKeyId, requestBodyHash } from "./persistence.js";
...
export async function removeCameraFromSite(input: RemoveCameraInput): Promise<RemoveCameraResult> {
  ...
  if (!camera.exists || camera.data()?.schemaVersion !== SCHEMA_VERSION || camera.data()?.siteId !== input.siteId || camera.data()?.status !== "active") throw new HttpError(404, "Active Camera not found.");
  ...
  const result = await firestore.runTransaction<RemoveCameraResult>(async (transaction) => {
    ...
    transaction.create(replacementMapRef, {
      ...map.data(), schemaVersion: SCHEMA_VERSION, revisionId: replacementMapRef.id, revisionNumber,
    ...
    transaction.set(firestore.collection("cameraRuntimeStates").doc(input.cameraId), {
      schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cameraId: input.cameraId, connectionStatus: "offline",
    ...
    if (dismissed.workCount > 0) enqueueOrchestratorTriggerInTransaction(transaction, {
    ...
    transaction.create(operationRef, {
      schemaVersion: SCHEMA_VERSION, siteId: input.siteId, actorUid: input.actor.uid, operation: "remove_camera",
  ...
  if (!result.replayed) {
    ...
    clearCameraVerificationCollectors(input.cameraId);
