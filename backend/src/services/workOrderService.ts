...
import { canonicalHash, assertExpectedRevision } from "./persistence.js";
...
export async function createManualWorkOrder(input: { siteId: string; title: string; instructions: string; severity: "warning" | "critical"; assignedCleanerId: string; target: { type: "camera"; cameraId: string } | { type: "coordinate"; point: MapPoint }; creationEvidenceMediaId?: string | null; idempotencyKey: string }, actor: WorkActor, requestId: string) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.create(workRef, data);
    transaction.create(firestore.collection("activeWorkOrderKeys").doc(hash("v2-active-manual-work", input.siteId, workId)), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, workOrderId: workId, cleanerId: input.assignedCleanerId, createdAt: FieldValue.serverTimestamp() });
...
export async function applyVerification(input: { siteId: string; workOrderId: string; outcome: "passed" | "failed" | "inconclusive"; reason?: string | null; expectedRevision: number; idempotencyKey: string; override?: boolean }, actor: WorkActor, requestId: string, command?: OrchestrationCommand) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    if (result.workStatus === "resolved") { transaction.update(cleanerRef, { activeWorkOrderId: null, activeWorkAssignedAt: null, lastResolvedWorkOrderId: input.workOrderId, lastResolvedWorkTarget: data.target ?? null, lastResolvedWorkAt: FieldValue.serverTimestamp(), lastResolvedMapRevisionId: data.mapRevisionId ?? null, updatedAt: FieldValue.serverTimestamp(), revision: FieldValue.increment(1) }); transaction.delete(firestore.collection("activeWorkOrderKeys").doc(hash("v2-active-work", input.siteId, data.alertId ?? input.workOrderId))); transaction.delete(firestore.collection("activeWorkOrderKeys").doc(hash("v2-active-manual-work", input.siteId, input.workOrderId))); enqueueOrchestratorTriggerInTransaction(transaction, { siteId: input.siteId, type: "retry_waiting_alerts", aggregateType: "work_order", aggregateId: input.workOrderId, triggerType: "cleaner_released", uniquenessKey: `cleaner-released:${input.workOrderId}` }); }
