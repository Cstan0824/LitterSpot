...
import { assertExpectedRevision, recordKeyHash } from "./persistence.js";
import { auditEventData, type AuditActor } from "./auditService.js";
import { deriveCleanerAvailability } from "./cleanerAvailability.js";
import { containingPolygon, pointInMapBounds, type MapPoint, type Polygon } from "./mapGeometry.js";
import { canTransitionWork, determineVerificationResult, requiresCompletionEvidence } from "./workPolicy.js";
import { getLiveIssueState, clearEvidenceCandidate } from "./liveMonitoringService.js";
...
import type { LiveObservation } from "./liveMonitoringService.js";
import { enqueueOrchestratorTriggerInTransaction } from "./orchestratorTriggers.js";
import { readOrchestrationGuard, commitOrchestration, OrchestrationConflict, type OrchestrationCommand } from "./orchestratorCommit.js";
import { serializeFirestore } from "./presentation.js";
import { createNotificationInTransaction, notifySiteSupervisors } from "./notificationService.js";
...
function workNotification(tx: Transaction, data: DocumentData, uid: string | undefined, workOrderId: string, type: string, eventKey: string, body: string) {
  if (!uid) return;
  createNotificationInTransaction(tx, {
...
export type WorkActor = AuditActor & { type: "supervisor" | "cleaner" | "orchestrator"; cleanerId?: string };
...
const hash = (namespace: string, ...values: unknown[]) => recordKeyHash(namespace, ...values);
...
export function clearCameraVerificationCollectors(cameraId: string) {
  cameraVerificationCollectors.delete(cameraId);
}

export function resetCameraVerificationCollectors() {
  cameraVerificationCollectors.clear();
}

export function inspectCameraVerificationCollectors(cameraId?: string) {
...
export async function recoverCameraVerificationCollectors() {
  resetCameraVerificationCollectors();
  ...
  for (const work of works.docs) {
    ...
    if (data.schemaVersion !== SCHEMA_VERSION || data.origin !== "alert" || data.target?.type !== "camera" || !data.cameraId || !data.latestVerificationId) continue;
...
function actorMap(actor: WorkActor) {
...
function present(id: string, data: DocumentData): WorkOrder {
  return {
    id,
    ...data,
...
function syncCameraWorkRuntime(work: WorkOrder) {
...
export async function createAlertWorkOrder(input: { siteId: string; alertId: string; assignedCleanerId: string; idempotencyKey: string }, actor: WorkActor, requestId: string, command?: OrchestrationCommand) {
  const workId = hash("work-order", input.siteId, "alert", input.alertId, input.idempotencyKey);
  const requestFingerprint = hash("work-request", input);
  const workRef = firestore.collection("workOrders").doc(workId);
  const activeKeyRef = firestore.collection("activeWorkOrderKeys").doc(hash("active-work", input.siteId, input.alertId));
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.set(firestore.collection("cameraRuntimeStates").doc(String(alertData.cameraId)), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cameraId: String(alertData.cameraId), cleanlinessState: "cleaning_in_progress", updatedAt: FieldValue.serverTimestamp() }, { merge: true });
    transaction.create(workRef.collection("events").doc(), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, workOrderId: workId, type: "assigned", fromStatus: "waiting_for_cleaner", toStatus: "assigned", cleanerId: input.assignedCleanerId, previousCleanerId: null, actor: actorMap(actor), reasonCode: "cleaner_assigned", note: null, evidenceMediaIds: [], requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null });
    transaction.create(alert.ref.collection("events").doc(), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, alertId: input.alertId, type: "assigned", fromStatus: "waiting_for_cleaner", toStatus: "assigned", workOrderId: workId, actor: actorMap(actor), reasonCode: "cleaner_assigned", note: null, requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null });
    workNotification(transaction, data, String(cleaner.data()!.authUid), workId, "work_assigned", workId, instructions);
    transaction.create(auditRef, auditEventData({ auditEventId: auditRef.id, actor, siteId: input.siteId, siteNameSnapshot: String(site.data()?.name), action: "work_order_assigned", resourceType: "WorkOrder", resourceId: workId, outcome: "succeeded", after: { alertId: input.alertId, cleanerId: input.assignedCleanerId }, requestId }));
  });
  if (replay) return serializeFirestore(replay);
  const created = await getWorkOrder(input.siteId, workId);
  syncCameraWorkRuntime(created);
  if (created.cameraId) publishCameraWorkflow(input.siteId, String(created.cameraId));
  return created;
}

export async function createManualWorkOrder(input: { siteId: string; title: string; instructions: string; severity: "warning" | "critical"; assignedCleanerId: string; target: { type: "camera"; cameraId: string } | { type: "coordinate"; point: MapPoint }; creationEvidenceMediaId?: string | null; idempotencyKey: string }, actor: WorkActor, requestId: string) {
  const workId = hash("work-order", input.siteId, "manual", input.idempotencyKey);
  const requestFingerprint = hash("work-request", input);
  const workRef = firestore.collection("workOrders").doc(workId);
  const auditRef = firestore.collection("auditEvents").doc();
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.create(workRef.collection("events").doc(), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, workOrderId: workId, type: "assigned", fromStatus: null, toStatus: "assigned", cleanerId: input.assignedCleanerId, previousCleanerId: null, actor: actorMap(actor), reasonCode: "manual_work_created", note: null, evidenceMediaIds: input.creationEvidenceMediaId ? [input.creationEvidenceMediaId] : [], requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null });
    workNotification(transaction, data, String(cleaner.data()!.authUid), workId, "work_assigned", workId, input.title);
    transaction.create(auditRef, auditEventData({ auditEventId: auditRef.id, actor, siteId: input.siteId, siteNameSnapshot: String(site.data()?.name), action: "manual_work_created", resourceType: "WorkOrder", resourceId: workId, outcome: "succeeded", after: { targetType: input.target.type, cleanerId: input.assignedCleanerId }, requestId }));
  });
  const created = await getWorkOrder(input.siteId, workId);
  syncCameraWorkRuntime(created);
  if (created.cameraId) publishCameraWorkflow(input.siteId, String(created.cameraId));
...
export async function getWorkOrder(siteId: string, workOrderId: string): Promise<WorkOrder> { const snapshot = await firestore.collection("workOrders").doc(workOrderId).get(); if (!snapshot.exists || snapshot.data()?.schemaVersion !== 2 || snapshot.data()?.siteId !== siteId) throw new HttpError(404, "Work Order not found."); return present(snapshot.id, snapshot.data()!); }

export async function listWorkOrdersPage(siteId: string, input: { status: string; cleanerId?: string; alertId?: string; zoneId?: string; cameraId?: string; origin?: "alert" | "manual"; limit: number; cursor?: string }) {
...
export async function listWorkOrders(siteId: string, input: { status: string; cleanerId?: string; alertId?: string; zoneId?: string; cameraId?: string; origin?: "alert" | "manual"; limit: number }) { return (await listWorkOrdersPage(siteId, input)).items; }
...
export async function transitionWorkOrder(siteId: string, workOrderId: string, next: "in_progress" | "awaiting_review", actor: WorkActor, input: { idempotencyKey: string; completionEvidenceMediaId?: string | null }, requestId: string) {
  const eventRef = firestore.collection("workOrders").doc(workOrderId).collection("events").doc(hash("work-event", workOrderId, input.idempotencyKey));
  await firestore.runTransaction(async (transaction) => {
    ...
    const verificationRef = next === "awaiting_review" ? workRef.collection("verifications").doc(hash("verification-request", workOrderId, input.idempotencyKey)) : null;
    if (verificationRef) transaction.create(verificationRef, { schemaVersion: SCHEMA_VERSION, siteId, workOrderId, alertId: data.alertId ?? null, kind: manualWork ? "manual_supervisor" : "camera_deterministic", status: manualWork ? "ready" : "collecting", requestedAt: FieldValue.serverTimestamp(), requestedBy: actorMap(actor), requiredSampleCount: manualWork ? null : verificationSampleCount(String(data.issueType)), acceptedSampleCount: 0, sampleSummaries: [], outcome: null, outcomeReasonCodes: [], completionEvidenceMediaId: input.completionEvidenceMediaId ?? null, decidedAt: null, decidedBy: null, override: null, appliedAt: null, requestId });
    ...
    if (data.cameraId) transaction.set(firestore.collection("cameraRuntimeStates").doc(String(data.cameraId)), { schemaVersion: SCHEMA_VERSION, siteId, cameraId: String(data.cameraId), cleanlinessState: next === "awaiting_review" ? "awaiting_review" : "cleaning_in_progress", updatedAt: FieldValue.serverTimestamp() }, { merge: true });
    transaction.create(eventRef, { schemaVersion: SCHEMA_VERSION, siteId, workOrderId, type: next === "in_progress" ? "started" : "submitted", fromStatus: from, toStatus: next, cleanerId: String(data.assignedCleanerId), previousCleanerId: null, actor: actorMap(actor), reasonCode: next, note: null, evidenceMediaIds: input.completionEvidenceMediaId ? [input.completionEvidenceMediaId] : [], requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null });
    void cleaner; void cleanerRef;
  });
  const updated = await getWorkOrder(siteId, workOrderId);
...
export async function applyVerification(input: { siteId: string; workOrderId: string; outcome: "passed" | "failed" | "inconclusive"; reason?: string | null; expectedRevision: number; idempotencyKey: string; override?: boolean }, actor: WorkActor, requestId: string, command?: OrchestrationCommand) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    const verificationRef = workRef.collection("verifications").doc(String(data.latestVerificationId ?? hash("verification", input.workOrderId, input.idempotencyKey)));
    ...
    const result = String(data.managementMode) === "manual" && actor.type === "supervisor"
      ...
      : determineVerificationResult({ outcome: input.outcome, managementMode: String(data.managementMode) as any, origin: String(data.origin) as any });
    ...
    if (actor.type === "supervisor" && !input.override) {
      const auditRef = firestore.collection("auditEvents").doc();
      transaction.create(auditRef, auditEventData({ auditEventId: auditRef.id, actor, siteId: input.siteId, action: "work_verification_applied", resourceType: "WorkOrder", resourceId: input.workOrderId, outcome: "succeeded", after: { outcome: input.outcome }, requestId }));
    }
    ...
    else transaction.create(verificationRef, { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, workOrderId: input.workOrderId, alertId: data.alertId ?? null, kind: data.origin === "manual" ? "manual_supervisor" : "camera_deterministic", status: "applied", requestedAt: FieldValue.serverTimestamp(), requestedBy: actorMap(actor), requiredSampleCount: null, acceptedSampleCount: 0, sampleSummaries: [], outcome: input.outcome, outcomeReasonCodes: input.reason ? [input.reason] : [], completionEvidenceMediaId: data.completionEvidenceMediaId ?? null, decidedAt: FieldValue.serverTimestamp(), decidedBy: actorMap(actor), override: input.override ? { outcome: input.outcome, reason: input.reason } : null, appliedAt: FieldValue.serverTimestamp(), requestId });
    ...
    transaction.create(workRef.collection("events").doc(), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, workOrderId: input.workOrderId, type: eventType, fromStatus: "awaiting_review", toStatus: result.workStatus, cleanerId: String(data.assignedCleanerId), previousCleanerId: null, actor: actorMap(actor), reasonCode: input.outcome, note: input.reason ?? null, evidenceMediaIds: data.completionEvidenceMediaId ? [data.completionEvidenceMediaId] : [], requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null });
    if (alertRef && alert?.exists) {
      ...
      if (alertStatus === "resolved") transaction.delete(firestore.collection("activeAlertKeys").doc(hash("active-alert", input.siteId, data.cameraId, data.issueType)));
      transaction.create(alertRef.collection("events").doc(), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, alertId: data.alertId, type: alertStatus === "resolved" ? "resolved" : "awaiting_review", fromStatus: "awaiting_review", toStatus: alertStatus, workOrderId: input.workOrderId, actor: actorMap(actor), reasonCode: input.outcome, note: input.reason ?? null, requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null });
    }
    if (result.workStatus === "resolved") { transaction.update(cleanerRef, { activeWorkOrderId: null, activeWorkAssignedAt: null, lastResolvedWorkOrderId: input.workOrderId, lastResolvedWorkTarget: data.target ?? null, lastResolvedWorkAt: FieldValue.serverTimestamp(), lastResolvedMapRevisionId: data.mapRevisionId ?? null, updatedAt: FieldValue.serverTimestamp(), revision: FieldValue.increment(1) }); transaction.delete(firestore.collection("activeWorkOrderKeys").doc(hash("active-work", input.siteId, data.alertId ?? input.workOrderId))); transaction.delete(firestore.collection("activeWorkOrderKeys").doc(hash("active-manual-work", input.siteId, input.workOrderId))); enqueueOrchestratorTriggerInTransaction(transaction, { siteId: input.siteId, type: "retry_waiting_alerts", aggregateType: "work_order", aggregateId: input.workOrderId, triggerType: "cleaner_released", uniquenessKey: `cleaner-released:${input.workOrderId}` }); }
    if (data.cameraId) transaction.set(firestore.collection("cameraRuntimeStates").doc(String(data.cameraId)), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cameraId: String(data.cameraId), cleanlinessState: result.workStatus === "resolved" ? "clean" : result.workStatus === "in_progress" ? "cleaning_in_progress" : "awaiting_review", updatedAt: FieldValue.serverTimestamp() }, { merge: true });
    if (input.override) transaction.create(firestore.collection("auditEvents").doc(), auditEventData({ auditEventId: randomUUID(), actor, siteId: input.siteId, action: "verification_overridden", resourceType: "WorkOrder", resourceId: input.workOrderId, outcome: "succeeded", reason: input.reason, after: { outcome: input.outcome }, requestId }));
  });
  if (replay) return serializeFirestore(replay);
  const workOrder = await getWorkOrder(input.siteId, input.workOrderId);
  ...
  if (workOrder.cameraId && workOrder.status === "resolved") clearEvidenceCandidate(input.siteId, workOrder.cameraId, workOrder.issueType);
...
export async function dismissWorkOrder(input: { siteId: string; workOrderId: string; reason: string; expectedRevision: number; idempotencyKey: string }, actor: WorkActor, requestId: string) {
  await firestore.runTransaction(async (transaction) => { const context = await workContext(input.siteId, input.workOrderId, transaction); const { data, workRef, alert, alertRef, cleanerRef } = context; const eventRef = workRef.collection("events").doc(hash("dismiss", input.workOrderId, input.idempotencyKey)); const existingEvent = await transaction.get(eventRef); if (existingEvent.exists) return; assertExpectedRevision(data, input.expectedRevision); if (![...ACTIVE].includes(String(data.status) as any)) throw new HttpError(409, "Work Order is already terminal."); const auditRef = firestore.collection("auditEvents").doc(); workNotification(transaction, data, context.cleaner.data()?.authUid, input.workOrderId, "work_dismissed", eventRef.id, "Your cleaning task has been dismissed."); if (data.alertId && data.cameraId) transaction.delete(firestore.collection("activeAlertKeys").doc(hash("active-alert", input.siteId, data.cameraId, data.issueType))); transaction.update(workRef, { status: "dismissed", dismissedAt: FieldValue.serverTimestamp(), dismissedBy: actorMap(actor), dismissReason: input.reason, updatedAt: FieldValue.serverTimestamp(), revision: FieldValue.increment(1) }); transaction.create(eventRef, { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, workOrderId: input.workOrderId, type: "dismissed", fromStatus: data.status, toStatus: "dismissed", cleanerId: String(data.assignedCleanerId), previousCleanerId: null, actor: actorMap(actor), reasonCode: "supervisor_dismissed", note: input.reason, evidenceMediaIds: [], requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null }); transaction.update(cleanerRef, { activeWorkOrderId: null, activeWorkAssignedAt: null, updatedAt: FieldValue.serverTimestamp(), revision: FieldValue.increment(1) }); transaction.delete(firestore.collection("activeWorkOrderKeys").doc(hash("active-work", input.siteId, data.alertId ?? input.workOrderId))); transaction.delete(firestore.collection("activeWorkOrderKeys").doc(hash("active-manual-work", input.siteId, input.workOrderId))); if (alertRef && alert?.exists) transaction.update(alertRef, { status: "dismissed", activeWorkOrderId: null, dismissedAt: FieldValue.serverTimestamp(), dismissedBy: actorMap(actor), dismissReason: input.reason, updatedAt: FieldValue.serverTimestamp(), revision: FieldValue.increment(1) }); if (data.cameraId) transaction.set(firestore.collection("cameraRuntimeStates").doc(String(data.cameraId)), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cameraId: String(data.cameraId), cleanlinessState: "clean", updatedAt: FieldValue.serverTimestamp() }, { merge: true }); transaction.create(auditRef, auditEventData({ auditEventId: auditRef.id, actor, siteId: input.siteId, action: "work_order_dismissed", resourceType: "WorkOrder", resourceId: input.workOrderId, outcome: "succeeded", reason: input.reason, before: { status: data.status }, after: { status: "dismissed" }, requestId })); });
  const dismissed = await getWorkOrder(input.siteId, input.workOrderId);
...
export async function listVerifications(siteId: string, workOrderId: string) { return (await listVerificationsPage(siteId, workOrderId, { limit: 20 })).items; }

export async function uploadCompletionEvidence(input: { siteId: string; workOrderId: string; cleanerId: string; file: { buffer: Buffer; mimetype: string; originalname: string } }) {
  const work = await getWorkOrder(input.siteId, input.workOrderId); if (work.assignedCleanerId !== input.cleanerId || work.status !== "in_progress" || work.origin !== "manual") throw new HttpError(409, "Completion Evidence is only accepted for the assigned Manual Work Order in progress.");
  const detected = detectSupportedImage(input.file.buffer); validateDeclaredImageType(input.file.mimetype, detected.mimeType); const mediaId = randomUUID(); const storageKey = `media/${mediaId}/completion-evidence.${detected.extension}`; await writeMedia(storageKey, input.file.buffer); await firestore.collection("mediaAssets").doc(mediaId).create({ schemaVersion: SCHEMA_VERSION, mediaId, siteId: input.siteId, purpose: "work_completion_evidence", ownerType: "work_order", ownerId: input.workOrderId, cameraId: null, mimeType: detected.mimeType, originalFileName: input.file.originalname, byteSize: input.file.buffer.length, sha256: hash("media", input.file.buffer.toString("base64")), storageKey, storageStatus: "available", width: null, height: null, durationSeconds: null, capturedAt: FieldValue.serverTimestamp(), retentionClass: "operational", expiresAt: null, createdAt: FieldValue.serverTimestamp(), createdByUid: input.cleanerId, deletedAt: null, revision: 1 }); return { mediaId, workOrderId: input.workOrderId };
}

export async function recordCameraVerificationObservation(siteId: string, cameraId: string, observation: LiveObservation) {
  ...
  for (const collector of collectors) {
    ...
    const changed = await firestore.runTransaction(async tx => {
      ...
      if (data.managementMode === "orchestrated" && data.origin === "alert") enqueueOrchestratorTriggerInTransaction(tx, {
