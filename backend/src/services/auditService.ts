...
import { sanitizeAuditSummary } from "./persistence.js";
...
export async function writeAuditEvent(input: {
  ...
  await reference.create(auditEventData({ ...input, auditEventId: reference.id }));
  return reference.id;
}
...
export async function listAuditEventsPage(filters: { siteId?: string; actorUid?: string; actorRole?: "superadmin" | "supervisor"; limit?: number; cursor?: string } = {}) {
...
export async function listAuditEvents(filters: { siteId?: string; actorUid?: string; actorRole?: "superadmin" | "supervisor"; limit?: number } = {}) { return (await listAuditEventsPage({ ...filters, limit: filters.limit ?? 100 })).items; }
