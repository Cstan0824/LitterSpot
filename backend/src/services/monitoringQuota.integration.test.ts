...
import { resetLiveMemory, setLiveInferenceForTests } from "./liveMonitoringService.js";
import { recordCameraVerificationObservation } from "./workOrderService.js";
...
run("monitoring Firestore budget", () => {
  afterEach(() => {
    setLiveInferenceForTests(null);
  ...
  it("does not query Work Orders for an ordinary frame when no verification collector is active", async () => {
    ...
    try {
      ...
      const applied = await recordCameraVerificationObservation(
        `quota-site-${suffix}`,
        `quota-camera-${suffix}`,
        {
  ...
  it("uses no Firestore reads or writes for a warm ordinary monitoring sample", async () => {
    ...
    try {
      ...
      let spill = false;
      setLiveInferenceForTests(async () => ({ image: { width: 640, height: 480 }, focusRegion: [], peopleCount: 0, people: [], bins: [], floorHazards: spill ? [{ className: "floor_spill", confidence: 0.9, bbox: { x1: 1, y1: 1, x2: 2, y2: 2 }, polygon: [] }] : [], modelVersions: {}, processingTimeMs: 1 } as any));
