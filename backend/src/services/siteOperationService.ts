...
import { recordSystemEvent } from "./systemService.js";
...
export async function getSiteOperation(siteId: string, operationId: string) {
...
export async function reconcileSiteOperation(siteId: string, operationId: string, pageSize = 25) {
  const opRef = firestore.collection("siteOperations").doc(operationId);
  const op = await getSiteOperation(siteId, operationId);
  ...
  for (const row of page.docs) {
    await firestore.runTransaction(async (tx) => {
      ...
      if (collection === "workOrders" && activeWork.includes(data.status)) {
        ...
        if (cleaner.data()?.siteId === siteId && cleaner.data()?.authUid) createNotificationInTransaction(tx, {
  ...
  return getSiteOperation(siteId, operationId);
}

export async function recoverSiteOperations() {
  ...
  for (const doc of ops.docs) {
    ...
    try { await reconcileSiteOperation(String(doc.data().siteId), doc.id); recovered += 1; }
    catch (error) {
      ...
      await recordSystemEvent(String(doc.data().siteId), "site_operation_failed").catch(() => undefined);
    }
  }
  return recovered;
}
