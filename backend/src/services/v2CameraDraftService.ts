...
export async function startV2CameraDraft(input: { siteId: string; kind: "create" | "reconfigure" | "physical_move"; cameraId?: string; name: string; description?: string | null; sourceType: CameraSourceType; placement?: { point: { xMeters: number; yMeters: number } } | null; provisionalZone?: { zoneId: string; zoneNameSnapshot: string; polygon: Polygon } | null; moveReason?: string | null; actorUid: string }) {
  ...
  const [revision, zoneSnapshot, previousPlacement, activeSource] = await Promise.all([activeRevision.get(), activeRevision.collection("zoneGeometry").get(), activeRevision.collection("cameraPlacements").doc(cameraId).get(), activeSourceRef?.get() ?? Promise.resolve(null)]);
  ...
  const preservedSource = input.kind === "physical_move" && activeSource?.exists ? activeSource.data() : null;
  const reference = firestore.collection("cameraDrafts").doc();
  const draftData = { schemaVersion: V2_SCHEMA_VERSION, draftId: reference.id, siteId: input.siteId, kind: input.kind, cameraId, baseCameraRevision: existing?.data()?.revision ?? null, baseMapRevisionId: String(site.data()?.activeMapRevisionId ?? ""), name: input.name.trim(), description: input.description?.trim() || null, placement, previousPlacement: input.kind === "physical_move" && previousPlacement.exists ? { point: previousPlacement.data()?.point, zoneId: previousPlacement.data()?.zoneId ?? null } : null, moveReason: input.kind === "physical_move" ? input.moveReason?.trim() : null, provisionalZone: input.provisionalZone ?? null, source: { type: input.sourceType, sourceMediaId: preservedSource?.sourceMediaId ?? null, browserDeviceHint: preservedSource?.browserDeviceHint ?? null, sampleIntervalSeconds: Number(preservedSource?.sampleIntervalSeconds ?? site.data()?.defaultSampleIntervalSeconds ?? 1), isSimulation: input.sourceType === "looped_video" }, registration: null, validationStatus: "not_validated", validationErrors: [], referenceCapturedAt: null, status: "draft", createdAt: FieldValue.serverTimestamp(), createdByUid: input.actorUid, updatedAt: FieldValue.serverTimestamp(), updatedByUid: input.actorUid, revision: 1 };
