...
export async function publishV2CameraDraft(draftId: string, actor: AuditActor, requestId: string) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    if (mapRevisionRef && mapZones && mapCameras && mapCleaners) {
      ...
      transaction.create(mapRevisionRef.collection("cameraPlacements").doc(cameraId), { schemaVersion: V2_SCHEMA_VERSION, siteId: data.siteId, cameraId, cameraNameSnapshot: data.name, point: data.placement.point, zoneId: data.placement.zoneId, zoneNameSnapshot: String(placementZone?.zoneNameSnapshot ?? data.placement.zoneId), publishedAt: FieldValue.serverTimestamp(), publishedByUid: actorUid });
