...
import { reconcileSiteOperation } from "./siteOperationService.js";
  ...
  try {
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      transaction.create(mapRef, {
        schemaVersion: SCHEMA_VERSION, revisionId: mapRef.id, siteId: siteRef.id, revisionNumber: 1, parentRevisionId: null,
      ...
      transaction.create(firestore.collection("userAccounts").doc(rootUid!), {
        schemaVersion: SCHEMA_VERSION, uid: rootUid, role: "supervisor", siteId: siteRef.id, profileId: rootUid, authority: "root",
      ...
      transaction.create(firestore.collection("supervisors").doc(rootUid!), {
        schemaVersion: SCHEMA_VERSION, uid: rootUid, siteId: siteRef.id, authority: "root", fullName: input.rootDisplayName.trim(),
    ...
    await writeAuditEvent({ actor, siteId: null, siteNameSnapshot: input.name.trim(), action: "site_created", resourceType: "Site", outcome: "failed", errorCode: "site_root_provision_failed", requestId }).catch(() => undefined);
    throw error;
  }
}

export async function updateSiteStatus(siteId: string, status: "active" | "inactive", reason: string, actor: AuditActor, requestId: string) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.create(operationRef, {
      schemaVersion: SCHEMA_VERSION, operationId: operationRef.id, siteId, type: status === "inactive" ? "deactivate" : "reactivate",
  ...
  if (status === "inactive") {
    for (let page = 0; page < 12; page += 1) {
      try { if ((await reconcileSiteOperation(siteId, operationRef.id)).status === "completed") break; }
...
export async function recoverRoot(input: {
  ...
  if (input.mode === "reset_existing") {
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      transaction.create(auditRef, auditEventData({ auditEventId: auditRef.id, actor, siteId: input.siteId, siteNameSnapshot: String(site.data()?.name), action: "root_recovered", resourceType: "Supervisor", resourceId: currentRootUid, outcome: "succeeded", reason: input.reason, after: { mode: input.mode, status: "active" }, requestId }));
    });
  ...
  try {
    ...
    await writeAuditEvent({ actor, siteId: input.siteId, siteNameSnapshot: String(site.data()?.name), action: "root_replaced", resourceType: "Supervisor", outcome: "failed", reason: input.reason, errorCode: "root_recovery_failed", requestId }).catch(() => undefined);
    throw error;
  }
}
