...
export function resetV2AlertQualification(cameraId?: string) {
...
export async function evaluateV2AlertsForCamera(siteId: string, cameraId: string, episodeId: string) {
  ...
  for (const issueType of ["floor_litter", "floor_spill", "bin_service"] as const) {
    ...
    const flagId = canonicalHash("v2-flag", cameraId, current.sampleId, issueType); const flagRef = firestore.collection("flags").doc(flagId);
    const flagData = { schemaVersion: V2_SCHEMA_VERSION, flagId, siteId, mapRevisionId: episode.mapRevisionId, zoneId: episode.zoneId, zoneNameSnapshot: episode.zoneName, cameraId, cameraNameSnapshot: episode.cameraName, registrationRevisionId: episode.registrationRevisionId, monitoringEpisodeId: episodeId, sampleId: current.sampleId, issueType, observedCondition: best.condition, severityCandidate: severity, confidence: best.confidence, magnitude: { issueCount: matches.length }, affectedBinIds: matches.filter((item) => issueType === "bin_service").map((item) => item.entityId), detections: matches.map((item) => ({ entityId: item.entityId, confidence: item.confidence, condition: item.condition, geometry: item.geometry })), modelVersions: current.modelVersions, qualificationPolicyVersion: "cleanliness-v2", qualificationSnapshot: { threshold, issueCount: matches.length, observationCount: observations.length }, capturedAt: Timestamp.fromMillis(current.capturedAtMs), isSimulation: current.isSimulation, alertId: null, createdAt: FieldValue.serverTimestamp() };
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      const evidenceValue = evidenceMediaId && candidate ? { mediaId: evidenceMediaId, flagId: canonicalHash("v2-flag", cameraId, candidate.observation.sampleId, issueType), capturedAt: Timestamp.fromMillis(candidate.observation.capturedAtMs), confidence: candidate.confidence, width: candidate.observation.image?.width ?? null, height: candidate.observation.image?.height ?? null, detections: candidate.observation.issues, people: candidate.observation.people, bins: candidate.observation.bins, observation: candidate.observation, coordinateSpace: "image_pixels", modelVersions: candidate.observation.modelVersions, selectionPolicyVersion: "highest-confidence-v2" } : prior?.evidence ?? null;
      transaction.set(alertRef, { schemaVersion: V2_SCHEMA_VERSION, alertId, siteId, mapRevisionId: episode.mapRevisionId, zoneId: episode.zoneId, zoneNameSnapshot: episode.zoneName, cameraId, cameraNameSnapshot: episode.cameraName, registrationRevisionId: episode.registrationRevisionId, issueType, observedCondition: best.condition, status: prior?.status ?? "waiting_for_cleaner", severity: nextSeverity, highestSeverity: nextSeverity, priorityScore: priorityScore({ severity: nextSeverity, createdAtMs: prior?.createdAt?.toMillis?.() ?? current.capturedAtMs, nowMs: current.capturedAtMs }), priorityPolicyVersion: "priority-v2", nextEscalationAt: nextSeverity === "warning" ? Timestamp.fromMillis((prior?.createdAt?.toMillis?.() ?? current.capturedAtMs) + 15 * 60000) : null, firstDetectedAt: prior?.firstDetectedAt ?? Timestamp.fromMillis(current.capturedAtMs), lastDetectedAt: Timestamp.fromMillis(current.capturedAtMs), occurrenceCount, affectedBinIds: [...new Set([...(prior?.affectedBinIds ?? []), ...matches.filter((item) => issueType === "bin_service").map((item) => item.entityId)])], evidence: evidenceValue, activeWorkOrderId: prior?.activeWorkOrderId ?? null, managementMode: prior?.managementMode ?? "orchestrated", isSimulation: current.isSimulation, resolvedAt: null, resolvedBy: null, dismissedAt: null, dismissedBy: null, dismissReason: null, createdAt: prior?.createdAt ?? FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp(), revision: Number(prior?.revision ?? 0) + 1 }, { merge: false });
