...
import { priorityScore, type V2IssueType } from "./v2AlertPolicy.js";
...
function expectedConditions(issueType: V2IssueType): SimulatedCondition[] {
  if (issueType === "floor_litter") return ["litter"];
  if (issueType === "floor_spill") return ["spill"];
  return ["full", "overflow"];
}

export async function createV2SimulatedAlert(input: {
  siteId: string;
  cameraId: string;
  issueType: V2IssueType;
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.create(activeKeyRef, { schemaVersion: V2_SCHEMA_VERSION, siteId: input.siteId, cameraId: input.cameraId, issueType: input.issueType, alertId, createdAt: FieldValue.serverTimestamp() });
    ...
    transaction.create(alertRef.collection("occurrences").doc(flagId), { schemaVersion: V2_SCHEMA_VERSION, siteId: input.siteId, alertId, flagId, capturedAt: FieldValue.serverTimestamp(), confidence: input.confidence, observedCondition: input.condition, severityCandidate: input.severity, becameEvidence: false, createdAt: FieldValue.serverTimestamp() });
    transaction.create(alertRef.collection("events").doc(canonicalHash("v2-simulated-alert-event", alertId)), { schemaVersion: V2_SCHEMA_VERSION, siteId: input.siteId, alertId, type: "created", fromStatus: null, toStatus: "waiting_for_cleaner", fromSeverity: null, toSeverity: input.severity, workOrderId: null, actor: { type: "human", uid: actor.uid, role: actor.role, authority: actor.authority ?? null, displayNameSnapshot: actor.displayName }, reasonCode: "simulated_test_alert", note: null, requestId, occurredAt: FieldValue.serverTimestamp(), analyticsAppliedVersion: null, analyticsAppliedAt: null });
    ...
    enqueueV2OrchestratorTriggerInTransaction(transaction, { siteId: input.siteId, type: "assign_alert", aggregateType: "alert", aggregateId: alertId, triggerType: "simulated_alert_created", uniquenessKey: alertId });
    transaction.set(firestore.collection("cameraRuntimeStates").doc(input.cameraId), { schemaVersion: V2_SCHEMA_VERSION, siteId: input.siteId, cameraId: input.cameraId, cleanlinessState: "alerted", updatedAt: FieldValue.serverTimestamp() }, { merge: true });
  });

  if (!idempotent && (await firestore.collection("orchestratorConfigs").doc(input.siteId).get()).data()?.status === "paused") {
    await notifyV2SiteSupervisors({
      siteId: input.siteId,
