...
import { cameraIdentityChangeAllowed, publishCameraState, validateCameraSource, type CameraSourceType } from "./cameraPolicy.js";
import { cameraRegistrationDraftSchema } from "../schemas/cameraRegistration.js";
import { containingPolygon, polygonArea, validateMapGeometry, type Polygon } from "./mapGeometry.js";
...
import { dismissActiveCameraOperations } from "./cameraMovementOperations.js";
import { clearCameraVerificationCollectors } from "./workOrderService.js";
...
export async function getCameraDraftForCamera(siteId: string, cameraId: string) {
...
export async function startCameraDraft(input: { siteId: string; kind: "create" | "reconfigure" | "physical_move"; cameraId?: string; name: string; description?: string | null; sourceType: CameraSourceType; placement?: { point: { xMeters: number; yMeters: number } } | null; provisionalZone?: { zoneId: string; zoneNameSnapshot: string; polygon: Polygon } | null; moveReason?: string | null; actorUid: string }) {
  ...
  const draftData = { schemaVersion: SCHEMA_VERSION, draftId: reference.id, siteId: input.siteId, kind: input.kind, cameraId, baseCameraRevision: existing?.data()?.revision ?? null, baseMapRevisionId: String(site.data()?.activeMapRevisionId ?? ""), name: input.name.trim(), description: input.description?.trim() || null, placement, previousPlacement: input.kind === "physical_move" && previousPlacement.exists ? { point: previousPlacement.data()?.point, zoneId: previousPlacement.data()?.zoneId ?? null } : null, moveReason: input.kind === "physical_move" ? input.moveReason?.trim() : null, provisionalZone: input.provisionalZone ?? null, source: { type: input.sourceType, sourceMediaId: preservedSource?.sourceMediaId ?? null, browserDeviceHint: preservedSource?.browserDeviceHint ?? null, sampleIntervalSeconds: Number(preservedSource?.sampleIntervalSeconds ?? site.data()?.defaultSampleIntervalSeconds ?? 1), isSimulation: input.sourceType === "looped_video" }, referenceMediaId: preservedRegistration?.referenceMediaId ?? null, registration: preservedRegistration ? { schemaVersion: preservedRegistration.schemaVersion ?? SCHEMA_VERSION, referenceMediaId: preservedRegistration.referenceMediaId, referenceSource: preservedRegistration.referenceSource ?? { type: "image" }, sourceWidth: preservedRegistration.sourceWidth, sourceHeight: preservedRegistration.sourceHeight, walkableFloorPolygon: preservedRegistration.walkableFloorPolygon, bins: preservedRegistration.bins ?? [], quality: preservedRegistration.quality ?? {} } : null, validationStatus: "not_validated", validationErrors: [], referenceCapturedAt: preservedRegistration?.publishedAt ?? null, status: "draft", createdAt: FieldValue.serverTimestamp(), createdByUid: input.actorUid, updatedAt: FieldValue.serverTimestamp(), updatedByUid: input.actorUid, revision: 1 };
  if (input.kind === "create") await reference.create(draftData);
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      transaction.create(lockRef, { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cameraId, draftId: reference.id, kind: input.kind, createdAt: FieldValue.serverTimestamp(), createdByUid: input.actorUid });
    });
  }
  return present(reference.id, (await reference.get()).data()!);
}

export async function cancelCameraDraft(siteId: string, draftId: string, actor?: AuditActor, requestId?: string) {
...
export async function publishCameraDraft(draftId: string, actor: AuditActor, requestId: string) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.set(sourceRef, { schemaVersion: SCHEMA_VERSION, sourceRevisionId: sourceRef.id, siteId: data.siteId, cameraId, revisionNumber: Number(existing.data()?.sourceRevisionNumber ?? 0) + 1, type: data.source.type, sourceMediaId: data.source.sourceMediaId ?? null, browserDeviceHint: data.source.browserDeviceHint ?? null, sampleIntervalSeconds: data.source.sampleIntervalSeconds, isSimulation: Boolean(data.source.isSimulation), publishedAt: FieldValue.serverTimestamp(), publishedByUid: actorUid, contentHash: `${cameraId}:${sourceRef.id}` });
    transaction.set(registrationRef, { schemaVersion: SCHEMA_VERSION, registrationRevisionId: registrationRef.id, siteId: data.siteId, cameraId, sourceRevisionId: sourceRef.id, revisionNumber: Number(existing.data()?.registrationRevisionNumber ?? 0) + 1, referenceMediaId: data.registration.referenceMediaId, referenceSource: {}, sourceWidth: data.registration.sourceWidth, sourceHeight: data.registration.sourceHeight, walkableFloorPolygon: data.registration.walkableFloorPolygon, bins: data.registration.bins ?? [], quality: data.registration.quality ?? {}, validation: { status: "valid" }, contentHash: `${cameraId}:${registrationRef.id}`, publishedAt: FieldValue.serverTimestamp(), publishedByUid: actorUid });
    transaction.set(firestore.collection("cameraRegistrations").doc(cameraId), { schemaVersion: SCHEMA_VERSION, siteId: data.siteId, cameraId, activeRegistrationRevisionId: registrationRef.id, revisionNumber: Number(existing.data()?.registrationRevisionNumber ?? 0) + 1, status: "ready", referenceMediaId: data.registration.referenceMediaId, binCount: (data.registration.bins ?? []).length, updatedAt: FieldValue.serverTimestamp(), updatedByUid: actorUid });
    transaction.set(targetCameraRef, { schemaVersion: SCHEMA_VERSION, cameraId, siteId: data.siteId, name: data.name, nameNormalized: String(data.name).toLowerCase(), description: data.description ?? null, status: existing.data()?.status ?? state.status, monitoringEnabled: existing.data()?.status === "inactive" ? false : state.monitoringEnabled, activeSourceRevisionId: sourceRef.id, activeRegistrationRevisionId: registrationRef.id, sourceType: data.source.type, isSimulation: state.isSimulation, createdAt: existing.data()?.createdAt ?? FieldValue.serverTimestamp(), createdByUid: existing.data()?.createdByUid ?? actorUid, updatedAt: FieldValue.serverTimestamp(), updatedByUid: actorUid, deactivatedAt: existing.data()?.deactivatedAt ?? null, deactivatedByUid: existing.data()?.deactivatedByUid ?? null, revision: Number(existing.data()?.revision ?? 0) + 1 });
    if (mapRevisionRef && mapZones && mapCameras && mapCleaners) {
      ...
      if (provisionalZone) {
        ...
        transaction.create(mapRevisionRef.collection("zoneGeometry").doc(provisionalZone.zoneId), { schemaVersion: SCHEMA_VERSION, siteId: data.siteId, zoneId: provisionalZone.zoneId, zoneNameSnapshot: provisionalZone.zoneNameSnapshot, polygon: provisionalZone.polygon, centroid, areaSquareMeters: polygonArea(provisionalZone.polygon), publishedAt: FieldValue.serverTimestamp(), publishedByUid: actorUid });
        transaction.set(firestore.collection("zones").doc(provisionalZone.zoneId), { schemaVersion: SCHEMA_VERSION, zoneId: provisionalZone.zoneId, siteId: data.siteId, name: provisionalZone.zoneNameSnapshot, nameNormalized: provisionalZone.zoneNameSnapshot.toLowerCase(), description: null, lifecycleStatus: "active", createdAt: FieldValue.serverTimestamp(), createdByUid: actorUid, updatedAt: FieldValue.serverTimestamp(), updatedByUid: actorUid, retiredAt: null, retiredByUid: null, revision: 1 });
      }
      const placementZone = provisionalZone && provisionalZone.zoneId === data.placement.zoneId
        ? { zoneNameSnapshot: provisionalZone.zoneNameSnapshot }
        : mapZones.docs.find((zone) => zone.id === data.placement.zoneId)?.data();
      ...
      transaction.create(mapRevisionRef.collection("cameraPlacements").doc(cameraId), { schemaVersion: SCHEMA_VERSION, siteId: data.siteId, cameraId, cameraNameSnapshot: data.name, point: data.placement.point, zoneId: data.placement.zoneId, zoneNameSnapshot: String(placementZone?.zoneNameSnapshot ?? data.placement.zoneId), publishedAt: FieldValue.serverTimestamp(), publishedByUid: actorUid });
  ...
  if (draftData.kind === "physical_move") {
    clearCameraVerificationCollectors(String(draftData.cameraId));
    if (dismissedWorkCount > 0) await enqueueImmediateAssignmentTrigger(String(draftData.siteId), "cleaner_released_by_camera_move", `camera-move:${draftId}`);
  }
