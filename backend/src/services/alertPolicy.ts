export type IssueType = "floor_litter" | "floor_spill" | "bin_service";
export type AlertStatus = "waiting_for_cleaner" | "assigned" | "in_progress" | "awaiting_review" | "resolved" | "dismissed";
export type Observation = { positive: boolean; capturedAtMs: number; severity?: "warning" | "critical" };

export function qualifiesIssue(issueType: IssueType, observations: Observation[], nowMs = Number.POSITIVE_INFINITY) {
...
export function nextAlertStatus(current: AlertStatus, next: AlertStatus) {
  const order: AlertStatus[] = ["waiting_for_cleaner", "assigned", "in_progress", "awaiting_review", "resolved"];
