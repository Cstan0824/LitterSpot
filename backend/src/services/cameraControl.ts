...
export async function controlCamera(input: { siteId: string; cameraId: string; expectedRevision: number; monitoringEnabled: boolean; deactivate?: boolean; actor: AuditActor; requestId: string }) {
  ...
  const result = await firestore.runTransaction(async tx => {
    ...
    tx.create(auditRef, auditEventData({ auditEventId: auditRef.id, siteId: input.siteId, siteNameSnapshot: String(site.data()?.name ?? ""), actor: input.actor, action: input.deactivate ? "camera_deactivated" : enabled ? "camera_monitoring_enabled" : "camera_monitoring_disabled", resourceType: "Camera", resourceId: ref.id, outcome: "succeeded", before: { monitoringEnabled: data.monitoringEnabled }, after: { monitoringEnabled: enabled }, requestId: input.requestId }));
