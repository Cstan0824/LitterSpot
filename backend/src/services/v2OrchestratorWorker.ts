...
import { runV2AssignmentCycle, runV2ReviewCycle, recoverV2OrchestratorRuns } from "./v2OrchestratorService.js";
import { enqueueV2ScheduledAssignmentTrigger } from "./v2OrchestratorTriggers.js";
import type { AssignmentSelector } from "./v2OrchestratorProvider.js";
...
async function scheduleWaitingSites() {
  ...
  await Promise.all(siteIds.map((siteId) => enqueueV2ScheduledAssignmentTrigger(siteId)));
}

export async function processV2OrchestratorTriggers(limit = 10, options: { selector?: AssignmentSelector; sleep?: (milliseconds: number) => Promise<void>; now?: Date; scheduleScan?: boolean; siteId?: string } = {}) {
    ...
    for (const event of candidates) {
      ...
      try {
        ...
        const result = claim.data.type === "review_work"
          ? await runV2ReviewCycle(String(claim.data.siteId), String(claim.data.aggregateId), runOptions)
          : await runV2AssignmentCycle(String(claim.data.siteId), runOptions);
...
export function startV2OrchestratorWorker() {
  if (timer) return;
  const tick = () => { void processV2OrchestratorTriggers().catch(() => console.error(JSON.stringify({ event: "orchestrator_worker_failed" }))); };
  tick();
  timer = setInterval(tick, 5_000);
  timer.unref();
}

export function stopV2OrchestratorWorker() {
