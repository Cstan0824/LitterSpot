...
import { containingPolygon, pointInMapBounds, polygonArea, validateMapGeometry, type MapPoint, type Polygon } from "./mapGeometry.js";
import { auditEventData, type AuditActor } from "./auditService.js";
import { publishSiteCameraControl } from "./cameraLiveEvents.js";
import { assertSiteBackground } from "./siteBackgroundService.js";
...
export async function startMapDraft(siteId: string, actor: AuditActor, requestId: string) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.create(auditRef, auditEventData({ auditEventId: auditRef.id, actor, siteId, siteNameSnapshot: String(latestSite.data()?.name ?? siteId), action: "site_map_draft_started", resourceType: "SiteMapDraft", resourceId: siteId, outcome: "succeeded", before: { activeMapRevisionId: revision.id }, after: { draftRevision: 1 }, requestId }));
  });
  return presentDraft(await readDraft(siteId));
}

export async function getMap(siteId: string, options: { allowInactive?: boolean } = {}) {
 ...
 */
export async function getCleanerMap(siteId: string, cleanerId: string) {
  const site = await firestore.collection("sites").doc(siteId).get();
...
export async function getMapDraft(siteId: string) {
...
export async function listRetiredZones(siteId: string) {
...
export async function listMapRevisions(siteId: string) {
  await siteForPrincipal(siteId);
  try {
...
export async function saveMapDraft(input: { siteId: string; baseRevisionId: string; expectedRevision?: number; widthMeters: number; heightMeters: number; gridSizeMeters: number; backgroundMediaId?: string | null; backgroundTransform?: SiteBackgroundTransform | null; zones: ZoneDraft[]; cameraPlacements?: PointDraft[]; cameraPlacementChanges?: CameraPlacementCorrection[]; cleanerStations?: PointDraft[]; actor: AuditActor; requestId: string }) {
  ...
  await Promise.all([
    ...
    assertSiteBackground(input.siteId, input.backgroundMediaId, input.backgroundTransform, { widthMeters: input.widthMeters, heightMeters: input.heightMeters }),
  ]);
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    for (const placement of input.cameraPlacements ?? []) {
      ...
      transaction.set(draftRef.collection("cameraPlacements").doc(placement.id), { ...oldCameraById.get(placement.id), schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cameraId: placement.id, point: placement.point, zoneId, zoneNameSnapshot: zoneId ? zoneNameById.get(zoneId) ?? zoneId : null, changeMode: correction?.mode ?? null, changeReason: correction?.reason ?? null, previousPoint: correction ? base?.point ?? null : null, previousZoneId: correction ? base?.zoneId ?? null : null, updatedAt: FieldValue.serverTimestamp(), updatedByUid: input.actor.uid });
    }
    for (const station of input.cleanerStations ?? []) transaction.set(draftRef.collection("cleanerStations").doc(station.id), { ...oldCleanerById.get(station.id), schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cleanerId: station.id, point: station.point, zoneId: containingPolygon(station.point, input.zones.map((zone) => ({ id: zone.zoneId, polygon: zone.polygon }))), updatedAt: FieldValue.serverTimestamp(), updatedByUid: input.actor.uid });
    const auditRef = firestore.collection("auditEvents").doc();
...
export async function validateMapDraft(siteId: string, actor: AuditActor, requestId: string) {
  const value = await readDraft(siteId);
  ...
  await Promise.all([
    ...
    assertSiteBackground(siteId, typeof value.data.backgroundMediaId === "string" ? value.data.backgroundMediaId : null, value.data.backgroundTransform as SiteBackgroundTransform | null | undefined, { widthMeters: Number(value.data.widthMeters), heightMeters: Number(value.data.heightMeters) }),
  ]);
  const site = await firestore.collection("sites").doc(siteId).get();
  const auditRef = firestore.collection("auditEvents").doc();
  await firestore.runTransaction(async (transaction) => {
...
export async function publishMapDraft(siteId: string, actor: AuditActor, requestId: string) {
  ...
  await Promise.all([
    ...
    assertSiteBackground(siteId, typeof data.backgroundMediaId === "string" ? data.backgroundMediaId : null, data.backgroundTransform as SiteBackgroundTransform | null | undefined, { widthMeters: Number(data.widthMeters), heightMeters: Number(data.heightMeters) }),
  ]);
  const siteRef = firestore.collection("sites").doc(siteId);
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    for (const zone of zones.docs) transaction.set(firestore.collection("zones").doc(zone.id), { schemaVersion: SCHEMA_VERSION, zoneId: zone.id, siteId, name: zone.data().zoneNameSnapshot, nameNormalized: String(zone.data().zoneNameSnapshot).toLowerCase(), description: null, lifecycleStatus: "active", createdAt: FieldValue.serverTimestamp(), createdByUid: actorUid, updatedAt: FieldValue.serverTimestamp(), updatedByUid: actorUid, retiredAt: null, retiredByUid: null, revision: FieldValue.increment(1) }, { merge: true });
...
export async function deleteMapDraft(siteId: string, actor: AuditActor, requestId: string) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.create(auditRef, auditEventData({ auditEventId: auditRef.id, actor, siteId, siteNameSnapshot: String(site.data()?.name), action: "site_map_draft_deleted", resourceType: "SiteMapDraft", resourceId: siteId, outcome: "succeeded", requestId }));
  });
  await firestore.recursiveDelete(draftRef).catch(() => undefined);
}

export async function publishCleanerStation(input: { siteId: string; cleanerId: string; point: MapPoint; actorUid: string; actorAuthority: "root" | "regular"; actorName: string; requestId: string }) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    transaction.create(revisionRef.collection("cleanerStations").doc(input.cleanerId), { schemaVersion: SCHEMA_VERSION, siteId: input.siteId, cleanerId: input.cleanerId, cleanerNameSnapshot: String(cleaner.data()?.fullName), point: input.point, zoneId, publishedAt: FieldValue.serverTimestamp(), publishedByUid: input.actorUid });
