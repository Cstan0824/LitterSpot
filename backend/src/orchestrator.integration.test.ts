...
import { createAlertWorkOrder, transitionWorkOrder, recordCameraVerificationObservation } from "./services/workOrderService.js";
import type { AssignmentSelector } from "./services/orchestratorProvider.js";
import { enqueueImmediateAssignmentTrigger } from "./services/orchestratorTriggers.js";
import { processOrchestratorTriggers } from "./services/orchestratorWorker.js";
...
async function seedScenario(options: { cleanerUnavailable?: boolean; recentMapMismatch?: boolean } = {}) {
  ...
  await firestore.collection("orchestratorConfigs").doc(siteId).set({ schemaVersion: 2, siteId, status: "running", assignmentEnabled: true, reviewEnabled: true, provider: "fake", model: "fake-pair-model", assignmentPolicyVersion: "bounded-pair-selection", reviewPolicyVersion: "fresh-evidence-review", technicalRetryLimit: 3, technicalRetryDelaysMs: [1000, 2000, 4000], requestTimeoutMs: 1000, lastRunAt: null, lastSuccessfulRunAt: null, lastFailureAt: null, lastFailureCode: null, revision: 1 });
...
run("Orchestrator integration", () => {
  ...
  it("builds pair context with Station and fresh recent Work distances", async () => {
    ...
    const context = await getAssignmentContext(scenario.siteId, { now: scenario.now });
  ...
  it("lets the model choose both the Alert and Cleaner from the supplied pairs", async () => {
    ...
    const result = await runAssignmentCycle(scenario.siteId, { selector, now: scenario.now, workerId: `worker-${randomUUID()}`, sleep: async () => undefined });
  ...
  it("retries provider failures at 1, 2, and 4 seconds without deterministic fallback", async () => {
    ...
    const result = await runAssignmentCycle(scenario.siteId, { selector, now: scenario.now, workerId: `worker-${randomUUID()}`, sleep: async (milliseconds) => { delays.push(milliseconds); } });
    expect(calls).toBe(4);
  ...
  it("leaves Alerts waiting when no Cleaner is available", async () => {
    ...
    const result = await runAssignmentCycle(scenario.siteId, { selector, now: scenario.now, workerId: `worker-${randomUUID()}` });
  ...
  it("exhausts invalid model selections without retaining the last invalid decision", async () => {
    ...
    const result = await runAssignmentCycle(scenario.siteId, { selector, now: scenario.now, sleep: async () => undefined });
    expect(calls).toBe(4);
  ...
  it("consumes a durable outbox trigger and runs assignment automatically", async () => {
    ...
    const eventId = await enqueueImmediateAssignmentTrigger(scenario.siteId, "integration_trigger", `integration:${randomUUID()}`);
    expect(await processOrchestratorTriggers(10, { selector, sleep: async () => undefined, now: scenario.now, scheduleScan: false, siteId: scenario.siteId })).toBeGreaterThan(0);
  ...
  it("uses the deterministic review tools and records the recent resolved Work target", async () => {
    ...
    const assignment = await runAssignmentCycle(scenario.siteId, { selector, now: scenario.now, workerId: `assignment-worker-${randomUUID()}`, sleep: async () => undefined });
  ...
  it("rejects assignment context while the Site Orchestrator is paused", async () => {
    ...
    await expect(getAssignmentContext(scenario.siteId, { now: scenario.now })).rejects.toMatchObject({ status: 409, message: "Orchestrator is paused." });
    await Promise.all(scenario.cleanerUids.map((uid) => firebaseAuth.deleteUser(uid).catch(() => undefined)));
  });

  it("serializes Site runs, binds their Site/worker and replays an atomic assignment after completion", async () => {
    const s = await seedScenario();
    const concurrent = await Promise.allSettled([createAssignmentRun(s.siteId, "owner"), createAssignmentRun(s.siteId, "owner")]);
    ...
    await expect(getAssignmentContextTool("other-site", runId, "owner")).rejects.toMatchObject({ status: 404 });
    await expect(getAssignmentContextTool(s.siteId, runId, "other-worker")).rejects.toMatchObject({ status: 409 });
    await getAssignmentContextTool(s.siteId, runId, "owner");
    ...
    await expect(assignCleanerByIdsTool({ ...command, alertId: "not-in-snapshot" })).rejects.toMatchObject({ status: 409 });
    const work = await assignCleanerByIdsTool(command);
    ...
    const replay = await assignCleanerByIdsTool(command);
    expect(replay.id).toBe(work.id);
    await expect(assignCleanerByIdsTool({ ...command, cleanerId: s.cleanerIds[1] })).rejects.toMatchObject({ status: 409 });
  ...
  it.each(["pause", "map", "manual", "expiry", "site"])("rejects %s changes during a provider call without assigning Work", async change => {
    ...
    const result = await runAssignmentCycle(s.siteId, { selector, sleep: async () => undefined });
  ...
  it("recovers expired runs and prevents the old worker from committing afterwards", async () => {
    const s = await seedScenario();
    const run = await createAssignmentRun(s.siteId, "old");
    await getAssignmentContextTool(s.siteId, run.runId, "old");
    ...
    await createAssignmentRun(s.siteId, "new");
    await expect(assignCleanerByIdsTool({ siteId: s.siteId, runId: run.runId, workerId: "old", alertId: s.alertIds[0], cleanerId: s.cleanerIds[0], rationaleSummary: "stale", requestId: "stale" })).rejects.toMatchObject({ status: 409 });
  ...
  it("queues fresh Camera verification, respects pause, then resolves through the review worker", async () => {
    ...
    const work = await createAlertWorkOrder({ siteId: s.siteId, alertId: s.alertIds[0], assignedCleanerId: s.cleanerIds[0], idempotencyKey: "review-queue" }, actor, "test");
    await transitionWorkOrder(s.siteId, work.id, "in_progress", actor, { idempotencyKey: "start-review" }, "test");
    await transitionWorkOrder(s.siteId, work.id, "awaiting_review", actor, { idempotencyKey: "submit-review" }, "test");
    ...
    expect(await recordCameraVerificationObservation(s.siteId, cameraId, sample)).toBe(0);
    for (let i = 0; i < 3; i++) await recordCameraVerificationObservation(s.siteId, cameraId, { ...sample, sampleId: `clear-${i}`, capturedAtMs: Date.now() + i });
    ...
    await expect(runReviewCycle(s.siteId, work.id)).rejects.toMatchObject({ status: 409 });
    await firestore.collection("orchestratorConfigs").doc(s.siteId).update({ status: "running" });
    await processOrchestratorTriggers(100, { scheduleScan: false, siteId: s.siteId, sleep: async () => undefined, selector: { async select() { throw new Error("No model required for review"); } } });
  ...
  it("recovers a trigger after the command committed but before trigger acknowledgement", async () => {
    const s = await seedScenario();
    const eventId = await enqueueImmediateAssignmentTrigger(s.siteId, "crash-test", randomUUID());
    ...
    const run = await createAssignmentRun(s.siteId, "crashed", "crash-test", eventId);
    await getAssignmentContextTool(s.siteId, run.runId, "crashed");
    const work = await assignCleanerByIdsTool({ siteId: s.siteId, runId: run.runId, workerId: "crashed", alertId: s.alertIds[0], cleanerId: s.cleanerIds[0], rationaleSummary: "Selected", requestId: "crash-test" });
    await event.update({ claimExpiresAt: Timestamp.fromMillis(0) });
    let called = false;
    await processOrchestratorTriggers(10, { siteId: s.siteId, scheduleScan: false, sleep: async () => undefined, selector: { async select() { called = true; throw new Error("Should not call"); } } });
  ...
  it("guards review outcome, manual takeover and replay without duplicate rework", async () => {
    const s = await seedScenario();
    const work = await createAlertWorkOrder({ siteId: s.siteId, alertId: s.alertIds[0], assignedCleanerId: s.cleanerIds[0], idempotencyKey: "review-test" }, { uid: "test", role: "supervisor", type: "orchestrator", displayName: "test" }, "test");
    ...
    const run = await createReviewRun(s.siteId, work.id, "reviewer");
    const command = { siteId: s.siteId, runId: run.runId, workOrderId: work.id, workerId: "reviewer", requestId: "test" };
    await expect(resolveVerifiedWork(command)).rejects.toMatchObject({ status: 409 });
    await workRef.update({ managementMode: "manual" });
    await expect(requestRework(command)).rejects.toMatchObject({ status: 409 });
    await workRef.update({ managementMode: "orchestrated" });
    const result = await requestRework(command);
    const replay = await requestRework(command);
