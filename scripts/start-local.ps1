...
$model = if ($env:MODEL_PATH) { $env:MODEL_PATH } else { Join-Path $root "runs\bin_overflow\yoloe26s_fused_tune20_v1\weights\best.pt" }
...
if (!(Test-Path $model)) { Write-Warning "No local model checkpoint was found. The dashboard will run, but FastAPI will report degraded until MODEL_PATH points to a local checkpoint." }
...
$pythonProcess = Start-ServiceProcess "fastapi" $python @("-m", "uvicorn", "app.main:app", "--app-dir", "ai-service", "--host", "127.0.0.1", "--port", "8000") @{ MODEL_PATH = $model; MODEL_VERSION = "yoloe26s-fused-tune20-v1"; DEVICE = "0"; INTERNAL_API_TOKEN = $token }
